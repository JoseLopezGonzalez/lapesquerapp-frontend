...
export function parseCajas(cajasString) {
  if (!cajasString || typeof cajasString !== 'string') {
    throw new ParsingError('String de cajas inválido o vacío', 'Cajas', cajasString);
  }

  const trimmed = cajasString.trim();
  if (trimmed === '') {
    throw new ParsingError('String de cajas vacío', 'Cajas', cajasString);
  }
  ...
  if (!tipo || tipo.trim() === '') {
    throw new ParsingError(`Tipo de cajas vacío. Valor: "${cajasString}"`, 'Cajas', cajasString);
  }
