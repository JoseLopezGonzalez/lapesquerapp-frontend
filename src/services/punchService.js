    ...
    .then(async (response) => {
      if (!response.ok) {
        ...
        const error = new Error(
          getErrorMessage(errorData) || 'Error al obtener estadísticas de trabajadores'
        );
        error.userMessage = errorData.userMessage || error.message;
        throw error;
      }
...
export async function createBulkPunches(punches, token) {
  try {
    ...
    if (response.status === 200 && data.message && !data.data) {
      const error = new Error(
        data.userMessage || data.message || 'Error al crear fichajes masivos'
      );
...
export async function validateBulkPunches(punches, token) {
  try {
    ...
    if (response.status === 404) {
      console.warn(
        'Endpoint de validación no disponible (404), usando mock con validación de integridad simulada'
      );
      return new Promise((resolve) => {
        setTimeout(() => {
    ...
    if (
      ...
      console.warn(
        'Endpoint de validación no disponible, usando mock con validación de integridad simulada'
      );
      return new Promise((resolve) => {
        setTimeout(() => {
          // Validación básica en el frontend + simulación de validación de integridad
...
export async function getPunchesByMonth(year, month, token, filters = {}) {
  try {
    ...
    const response = await fetchWithTenant(
      `${API_URL_V2}punches/calendar?${queryParams.toString()}`,
      {
        method: 'GET',
        headers: {
          'Content-Type': 'application/json',
