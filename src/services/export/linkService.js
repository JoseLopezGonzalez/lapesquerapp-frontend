...
export function groupLinkedSummaryBySupplier(linkedSummaryArray) {
  ...
  const groupedArray = Object.values(grouped).map((group) => {
    const barcoNombre = group.isGrouped ? group.barcoNombres.join(' + ') : group.barcoNombres[0];
...
export async function validatePurchases(linkedSummaryArray) {
  ...
  try {
    ...
    const res = await fetchWithTenant(
      `${API_URL_V2}raw-material-receptions/validate-bulk-update-declared-data`,
      {
        method: 'POST',
        headers: {
    ...
    if (responseData.results && Array.isArray(responseData.results)) {
      responseData.results.forEach((result) => {
        ...
        const linea = comprasValidas.find(
          (l) =>
            l.supplierId === result.supplier_id &&
    ...
    if (responseData.errors_details && Array.isArray(responseData.errors_details)) {
      responseData.errors_details.forEach((errorDetail) => {
        ...
        const linea = comprasValidas.find(
          (l) =>
            l.supplierId === errorDetail.supplier_id &&
...
export async function linkAllPurchases(linkedSummaryArray) {
  ...
  try {
    ...
    // Make bulk request
    const res = await fetchWithTenant(
      `${API_URL_V2}raw-material-receptions/bulk-update-declared-data`,
      {
        method: 'POST',
        headers: {
          'Content-Type': 'application/json',
    ...
    return {
      ...
      erroresDetalles: comprasValidas.map((linea) => ({
        ...
        error:
          ...
          error.response?.data?.userMessage ||
          error.message ||
          'Error al procesar la solicitud masiva',
      })),
    };
  }
}
