 ...
 */
export function getPaymentTermsOptions(token) {
    return fetchWithTenant(`${API_URL_V2}payment-terms/options`, {
        method: 'GET',
        headers: {
        ...
        .then((response) => {
            if (!response.ok) {
                return response.json().then((errorData) => {
                    throw new Error(errorData.message || 'Error al obtener método de pagos');
                });
            }
        ...
        .finally(() => {
            ...
            console.log('getPaymentTermsOptions finalizado');
        });
}
