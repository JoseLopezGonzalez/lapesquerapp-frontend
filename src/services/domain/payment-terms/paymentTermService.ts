/**
 * Service de dominio para Payment Terms (Formas de pago). Bloque 5.
 */
...
import type { PaymentTerm, CatalogListResponse, CatalogListFilters, CatalogOption } from '@/types/catalog';

const ENDPOINT = 'payment-terms';

export const paymentTermService = {
  async list(filters: CatalogListFilters = {}, pagination: { page?: number; perPage?: number } = {}): Promise<CatalogListResponse<PaymentTerm>> {
    ...
    return fetchEntitiesGeneric(url, token) as Promise<CatalogListResponse<PaymentTerm>>;
  },
  async getById(id: number | string): Promise<PaymentTerm> {
    const token = await getAuthToken();
    return fetchEntityDataGeneric(`${API_URL_V2}${ENDPOINT}/${id}`, token) as Promise<PaymentTerm>;
  },
  async create(data: Record<string, unknown>): Promise<PaymentTerm> {
    const token = await getAuthToken();
    const response = await createEntityGeneric(`${API_URL_V2}${ENDPOINT}`, data, token);
    const result = await response.json();
    return (result.data ?? result) as PaymentTerm;
  },
  async update(id: number | string, data: Record<string, unknown>): Promise<PaymentTerm> {
    const token = await getAuthToken();
