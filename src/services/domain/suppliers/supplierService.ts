/**
 * Service de dominio para Suppliers (Proveedores). Bloque 6.
 */
...
import type { Supplier, CatalogListResponse, CatalogListFilters, CatalogOption } from '@/types/catalog';

const ENDPOINT = 'suppliers';

export const supplierService = {
  async list(filters: CatalogListFilters = {}, pagination: { page?: number; perPage?: number } = {}): Promise<CatalogListResponse<Supplier>> {
    ...
    return fetchEntitiesGeneric(url, token) as Promise<CatalogListResponse<Supplier>>;
  },
  async getById(id: number | string): Promise<Supplier> {
    const token = await getAuthToken();
    return fetchEntityDataGeneric(`${API_URL_V2}${ENDPOINT}/${id}`, token) as Promise<Supplier>;
  },
  async create(data: Record<string, unknown>): Promise<Supplier> {
    ...
    return (result.data ?? result) as Supplier;
  },
  async update(id: number | string, data: Record<string, unknown>): Promise<Supplier> {
    const token = await getAuthToken();
