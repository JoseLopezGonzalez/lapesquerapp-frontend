...
export const fetchAutocompleteFilterOptions = async (endpoint) => {
  ...
  try {
    ...
    if (!response.ok && response.status === 404 && endpoint === 'external-users/options') {
      response = await fetchWithTenant(
        `${API_URL_V2}external-users?page=1&perPage=100`,
        requestOptions
      );
    }
    ...
    if (!response.ok) {
      ...
      throw new Error(
        getErrorMessage(errorData) ||
          `Error ${response.status}: Error al obtener opciones de autocompletado.`
      );
    }
