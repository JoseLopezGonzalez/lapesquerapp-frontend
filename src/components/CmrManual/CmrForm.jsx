...
export default function CmrForm({ value, onChange }) {
  ...
  const productLines = (() => {
    ...
    if (data.marksAndNumbers || data.numberOfPackages || data.methodOfPacking || data.natureOfGoods || data.grossWeight || data.volume) {
      return [{
  ...
  const removeProductLine = (lineIndex) => {
    ...
    set('productLines', productLines.filter((_, i) => i !== lineIndex));
  };

  const payment20Grid = (() => {
    const raw = data.payment20 ?? defaultCmrData.payment20;
    if (!Array.isArray(raw) || raw.length < 6) return defaultCmrData.payment20.map(row => [...row]);
    return raw.slice(0, 6).map(row => (Array.isArray(row) && row.length >= 6 ? [...row.slice(0, 6)] : [...Array(6).fill('')]));
  })();
  const setPayment20Cell = (row, col, value) => {
    const next = payment20Grid.map((r, ri) => (ri === row ? r.map((c, ci) => (ci === col ? value : c)) : [...r]));
    set('payment20', next);
  };
              ...
              <FieldContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <Input
                    ...
                    placeholder="Lugar"
                  />
                  <DatePicker date={date4} onChange={(d) => setPlaceDate4(place4, d)} formatStyle="short" />
                </div>
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
...
<TableHead>Marcas</TableHead>
              ...
              <TableBody>
                {productLines.map((line, idx) => (
                  <TableRow key={idx}>
                    <TableCell className="p-1"><Input className="h-8 text-xs" value={line.marksAndNumbers} onChange={(e) => setProductLineField(idx, 'marksAndNumbers', e.target.value)} placeholder="" /></TableCell>
                    <TableCell className="p-1"><Input type="number" min={0} className="h-8 text-xs w-14" value={line.numberOfPackages === 0 ? '' : line.numberOfPackages} onChange={(e) => setProductLineField(idx, 'numberOfPackages', e.target.value ? Number(e.target.value) : 0)} placeholder="0" /></TableCell>
                    <TableCell className="p-1"><Input className="h-8 text-xs" value={line.methodOfPacking} onChange={(e) => setProductLineField(idx, 'methodOfPacking', e.target.value)} placeholder="" /></TableCell>
                    <TableCell className="p-1"><Input className="h-8 text-xs" value={line.natureOfGoods} onChange={(e) => setProductLineField(idx, 'natureOfGoods', e.target.value)} placeholder="" /></TableCell>
                    <TableCell className="p-1"><Input className="h-8 text-xs" value={line.statisticalNumber} onChange={(e) => setProductLineField(idx, 'statisticalNumber', e.target.value)} placeholder="" /></TableCell>
                    <TableCell className="p-1"><Input type="number" min={0} step={0.01} className="h-8 text-xs w-16" value={line.grossWeight === 0 ? '' : line.grossWeight} onChange={(e) => setProductLineField(idx, 'grossWeight', e.target.value ? Number(e.target.value) : 0)} placeholder="0" /></TableCell>
                    <TableCell className="p-1"><Input className="h-8 text-xs" value={line.volume} onChange={(e) => setProductLineField(idx, 'volume', e.target.value)} placeholder="" /></TableCell>
                    <TableCell className="p-1">
                      <Button type="button" variant="outline" size="icon-sm" onClick={() => removeProductLine(idx)} disabled={productLines.length <= 1} title="Quitar línea" aria-label="Quitar línea"><Trash2 className="size-3.5" /></Button>
                    </TableCell>
                  </TableRow>
                ))}
              </TableBody>
            </Table>
            <Button type="button" variant="outline" size="sm" onClick={addProductLine}>+ Añadir línea de producto</Button>
          </FieldGroup>
        </CardContent>
      </Card>
        ...
        <CardContent>
          <FieldGroup className="space-y-4">
            <Field className={fieldClass}>
              ...
              <FieldContent>
                <Input className={inputClass} value={palletsPallets[0] ?? ''} onChange={(e) => setPalletsPallets(0, e.target.value)} placeholder="" />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
              <FieldLabel>Remesas al remitente</FieldLabel>
              <FieldContent>
                <Input className={inputClass} value={palletsPallets[1] ?? ''} onChange={(e) => setPalletsPallets(1, e.target.value)} placeholder="" />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
              <FieldLabel>Entregados al destinatario</FieldLabel>
              <FieldContent>
                <Input className={inputClass} value={palletsPallets[2] ?? ''} onChange={(e) => setPalletsPallets(2, e.target.value)} placeholder="" />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
              <FieldLabel>Devueltos por el destinatario</FieldLabel>
              <FieldContent>
                <Input className={inputClass} value={palletsPallets[3] ?? ''} onChange={(e) => setPalletsPallets(3, e.target.value)} placeholder="" />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
              <FieldLabel>No devueltos, a recoger</FieldLabel>
              <FieldContent>
                <Input className={inputClass} value={palletsPallets[4] ?? ''} onChange={(e) => setPalletsPallets(4, e.target.value)} placeholder="" />
              </FieldContent>
            </Field>
          </FieldGroup>
        </CardContent>
        ...
        <CardContent>
          <FieldGroup className="space-y-4">
            <Field className={fieldClass}>
              ...
              <FieldContent>
                <Textarea className={inputClass} value={data.senderInstructions ?? ''} onChange={(e) => set('senderInstructions', e.target.value)} placeholder="Instrucciones" rows={2} />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
              <FieldLabel>14. Forma de pago</FieldLabel>
              <FieldContent>
                <Input className={inputClass} value={data.methodOfPayment ?? ''} onChange={(e) => set('methodOfPayment', e.target.value)} placeholder="Forma de pago" />
              </FieldContent>
            </Field>
          </FieldGroup>
        </CardContent>
        ...
        <CardContent>
          <FieldGroup className="space-y-4">
            <Field className={fieldClass}>
              ...
              <FieldContent>
                <Textarea className={`${inputClass} resize-y`} value={data.carrier ?? ''} onChange={(e) => set('carrier', e.target.value)} placeholder="Porteador, dirección, país" rows={4} />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
              <FieldLabel>17. Porteadores sucesivos</FieldLabel>
              <FieldContent>
                <Textarea className={`${inputClass} resize-y`} value={data.successiveCarriers ?? ''} onChange={(e) => set('successiveCarriers', e.target.value)} placeholder="Porteadores sucesivos" rows={3} />
              </FieldContent>
            </Field>
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
              <Field className={fieldClass}>
                ...
                <FieldContent>
                  <Input value={data.distance ?? ''} onChange={(e) => set('distance', e.target.value)} placeholder="Distancia en km" />
                </FieldContent>
              </Field>
              <Field className={fieldClass}>
                <FieldLabel>17 Bis. Matrícula vehículo</FieldLabel>
                <FieldContent>
                  <Input value={data.vehicleRegistration ?? ''} onChange={(e) => set('vehicleRegistration', e.target.value)} placeholder="Matrícula vehículo" />
                </FieldContent>
              </Field>
              <Field className={fieldClass}>
                <FieldLabel>17 Bis. Matrícula remolque</FieldLabel>
                <FieldContent>
                  <Input value={data.trailerRegistration ?? ''} onChange={(e) => set('trailerRegistration', e.target.value)} placeholder="Remolque / semiremolque" />
                </FieldContent>
              </Field>
              <Field className={fieldClass}>
                <FieldLabel>Temperatura controlada (ºC)</FieldLabel>
                <FieldContent>
                  <Input value={data.controlledTemperature ?? ''} onChange={(e) => set('controlledTemperature', e.target.value)} placeholder="Ej. 2, 2 a 8, -18" />
                </FieldContent>
              </Field>
            </div>
            <Field className={fieldClass}>
              ...
              <FieldContent>
                <Input className={inputClass} value={data.reservationsOrObservations ?? ''} onChange={(e) => set('reservationsOrObservations', e.target.value)} placeholder="Reservas y observaciones del porteador" />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
              <FieldLabel>19. Estipulaciones particulares</FieldLabel>
              <FieldContent>
                <Textarea className={inputClass} value={data.specialAgreements ?? ''} onChange={(e) => set('specialAgreements', e.target.value)} placeholder="Estipulaciones" rows={2} />
              </FieldContent>
            </Field>
            <Field className={fieldClass}>
                ...
                <Table>
                  ...
                  <TableBody>
                    {[
                      ...
                      <TableRow key={row}>
                        <TableCell className="font-medium py-1">{label}</TableCell>
                        {[0, 1, 2, 3, 4, 5].map((col) => (
            ...
            <Field className={fieldClass}>
              ...
              <FieldContent>
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-2">
                  <Input value={place21} onChange={(e) => setPlaceDate21(e.target.value, date21)} placeholder="Lugar" />
                  <DatePicker date={date21} onChange={(d) => setPlaceDate21(place21, d)} formatStyle="short" />
                </div>
              </FieldContent>
            </Field>
          </FieldGroup>
