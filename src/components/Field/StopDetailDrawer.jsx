  ...
  return (
    <Drawer.Root open={open} onOpenChange={onOpenChange} shouldScaleBackground>
      <Drawer.Portal>
        ...
        <Drawer.Content
          className={cn(
            'border-border bg-background fixed right-0 bottom-0 left-0 z-30 mt-24 flex max-h-[78vh] flex-col rounded-t-[28px] border shadow-2xl',
            MOBILE_SAFE_AREAS.BOTTOM
          )}
        >
          <Drawer.Title className="sr-only">Detalle operativo de la parada</Drawer.Title>
          <Drawer.Description className="sr-only">
            Acciones principales para la parada enfocada.
          </Drawer.Description>
          ...
          <div className="flex min-h-0 flex-1 flex-col">
            <div className="border-b px-4 pt-4 pb-3">
              <div className="flex items-start justify-between gap-3">
                ...
                {focusedStop && (
                  <Badge className="shrink-0">{focusedStop.stopType || 'obligatoria'}</Badge>
                )}
              </div>
            </div>
            ...
            <div className="flex min-h-0 flex-1 flex-col overflow-hidden px-4 pb-3">
              {focusedStop && (
                <div className="space-y-2 py-3">
                  <div className="grid grid-cols-2 gap-2">
                    <Button
                      onClick={() =>
                        openInGoogleMaps(focusedStop.lat, focusedStop.lng, focusedStopQuery)
                      }
                      className="h-11 justify-between gap-2"
                    >
                      <span className="flex items-center gap-2">
                        <Image
                          src="/brands/google-maps.svg"
                          alt=""
                          width={20}
                          height={20}
                          className="shrink-0"
                      ...
                      <Link
                        href={{
                          ...
                          query: {
                            ...
                            ...(focusedStop.customerId
                              ? { customerId: focusedStop.customerId }
                              : {}),
                          },
                        }}
                      >
              ...
              {focusedStop && (
                <div className="grid grid-cols-2 gap-2 border-t pt-3">
                  ...
                  <Button
                    onClick={() =>
                      onOpenResultDialog(focusedStop, focusedStopOrder ? 'delivery' : 'visit')
                    }
                    className="h-11 justify-start gap-2"
                  >
