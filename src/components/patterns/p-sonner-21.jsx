...
function LoadingToast({ message }) {
  return (
    <div className="bg-popover text-popover-foreground border-border flex w-[356px] items-center gap-3 rounded-md border p-4 shadow-lg">
  ...
  return (
    <div className="bg-popover text-popover-foreground border-border flex w-[356px] items-start gap-3 rounded-md border p-4 shadow-lg">
      ...
      <div className="flex flex-1 flex-col gap-0.5">
        ...
        <p className="text-muted-foreground text-xs">3 files uploaded successfully.</p>
      </div>
    </div>
  );
}
...
export function Pattern() {
  ...
  const startProgress = () => {
    toastId.current = toast.custom(() => <LoadingToast message="Preparing upload..." />, {
      duration: Infinity,
    });
