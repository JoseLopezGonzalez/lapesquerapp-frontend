...
export function MessageList({ messages, isLoading }) {
  ...
  const hasMessageContent = (message) => {
    if (Array.isArray(message.parts) && message.parts.length > 0) {
      return message.parts.some((part) => part.type === 'text' && part.text && part.text.trim());
    }
                ...
                if (Array.isArray(message.parts) && message.parts.length > 0) {
                  return message.parts.map((part, idx) => {
                    ...
                    if (part.type === 'tool-call' || part.type === 'tool-invocation') {
                      return (
                        <div key={idx} className="border-border/50 mt-2 border-t pt-2">
                          <div className="flex items-center gap-1 text-xs opacity-70">
                            <Loader2 className="h-3 w-3 animate-spin" />
                            <span>
                              Consultando: {part.toolName || part.toolCallId || 'herramienta'}...
                            </span>
                          </div>
                        </div>
                      );
                      ...
                      return (
                        <div key={idx} className="border-border/50 mt-2 border-t pt-2">
                          <div className="text-xs opacity-70">
                            <div className="mb-1">
                              ✅ Consulta completada: {part.toolName || 'herramienta'}
                            </div>
                            {part.result && (
                              <details className="mt-1">
                                ...
                                <pre className="bg-muted/50 mt-1 max-h-32 overflow-auto rounded p-2 text-xs">
