...
export default function CostSourceSelector({
  ...
  const updateSource = (index, field, value) => {
    ...
    // Si se actualiza percentage, calcular weight_kg y limpiar el otro campo
      ...
      currentSource.contributed_weight_kg =
        percentage > 0 ? (percentage / 100) * totalWeightKg : null;
    }
  ...
  if (loading) {
    return <div className="py-4 text-center">Cargando fuentes...</div>;
  }
                ...
                <TableRow key={index}>
                  ...
                  <TableCell>
                    <Input
                      ...
                      onChange={(e) =>
                        updateSource(index, 'contribution_percentage', e.target.value)
                      }
                      className="w-24"
                    />
                  </TableCell>
            ...
            <SelectContent loading={loading}>
              {inputs.length > 0 && (
                <>
                  ...
                  {inputs.map((input) => (
                    <SelectItem
                      key={`stock_product-${input.productId}`}
                      value={`stock_product-${input.productId}`}
                    >
                      {input.product?.name || `Producto #${input.productId}`} -{' '}
                      {formatWeight(input.totalWeight)}
                    </SelectItem>
                  ))}
                </>
              )}
