...
function SortablePriorityItem({ item, usage, disabled = false, onClick }) {
  ...
  return (
    <div
      ...
      style={style}
      className={cn(
        'bg-background flex cursor-pointer items-center justify-between gap-3 rounded-md border px-3 py-2 text-xs select-none',
        isDragging ? 'ring-primary/30 shadow-md ring-1' : ''
      )}
      onClick={onClick}
    >
      <button
        type="button"
        className="text-muted-foreground cursor-grab active:cursor-grabbing disabled:cursor-default"
        {...attributes}
        {...listeners}
        disabled={disabled}
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-muted-foreground truncate">
          Restante: {formatWeight(usage?.remainingSourceWeight || 0)} /{' '}
...
function SortableOutputPriorityItem({ item, disabled = false, onClick }) {
  const { attributes, listeners, setNodeRef, transform, transition, isDragging } = useSortable({
    id: item.id,
    disabled,
  });
  ...
  return (
    ...
    >
      ...
      <div className="min-w-0 flex-1">
        ...
        <p className="text-muted-foreground truncate">Peso objetivo: {formatWeight(item.weight)}</p>
      </div>
    </div>
  );
}

const ProductionOutputsManager = ({
  productionRecordId,
  initialOutputs: initialOutputsProp = [],
  onRefresh,
  hideTitle = false,
  renderInCard = false,
  cardTitle,
  ...
  const getAdjustedOutputWeightFromSource = React.useCallback(
    (sourceWeight) => {
  ...
  const getSourceWeightFromAdjustedOutputWeight = React.useCallback(
    (adjustedOutputWeight) => {
  ...
  const sourceUsageMap = React.useMemo(() => {
    ...
    usage.forEach((entry) => {
      ...
      entry.totalTransformedWeight = roundToTwo(
        getAdjustedOutputWeightFromSource(entry.totalWeight)
      );
      ...
      entry.remainingSourceWeight = roundToTwo(
        Math.max(0, entry.totalWeight - entry.assignedSourceWeight)
      );
      entry.remainingTransformedWeight = roundToTwo(
        getAdjustedOutputWeightFromSource(entry.remainingSourceWeight)
      );
      entry.usagePercentage =
        entry.totalWeight > 0
          ? roundToTwo((entry.assignedSourceWeight / entry.totalWeight) * 100)
          : 0;
    });
  ...
  const getAvailableSourceWeightForLine = React.useCallback(
    (row, sourceIndex, sourceKey) => {
  ...
  const getOrderedPriorityKeys = React.useCallback(
    (row) => {
      const explicit = Array.isArray(row?.source_priority)
        ? row.source_priority.filter((key) => sourceOptionsMap.has(key))
        : [];
      const existing = Array.isArray(row?.sources)
        ? row.sources
            .map((source) => getSourceKey(source))
            .filter((key) => key && sourceOptionsMap.has(key))
        : [];
      const remaining = sourceOptions
        .map((option) => option.key)
        .filter((key) => !explicit.includes(key) && !existing.includes(key));
      return [...explicit, ...existing.filter((key) => !explicit.includes(key)), ...remaining];
    },
    [sourceOptions, sourceOptionsMap]
  );

  const setRowSourcePriority = React.useCallback(
    (rowId, orderedKeys) => {
  ...
  const recalculateSourcesForRow = React.useCallback(
    (row, sources) => {
      ...
      return (sources || []).map((source) => {
        ...
        return {
          ...
          contribution_percentage:
            outputWeight > 0 ? roundToTwo((transformedWeight / outputWeight) * 100) : null,
        };
      });
    },
  ...
  const clearRowSources = React.useCallback(
    (rowId) => {
  ...
  const distributeSourcesProportionally = React.useCallback(() => {
    ...
    allRows.forEach((row) => {
      ...
      updateRow(
        row.id,
        'source_priority',
        availableSources.map((option) => option.key)
      );
    });

    const totalAvailableSourceWeight = availableSources.reduce(
      (sum, source) => sum + source.totalWeight,
      0
    );
    const totalRequiredSourceWeight = validRows.reduce(
      (sum, row) =>
        sum + getSourceWeightFromAdjustedOutputWeight(parseFloat(row.weight_kg || 0) || 0),
      0
    );
    const coverageRatio =
      totalRequiredSourceWeight > 0
  ...
  const distributeMatchingSourcesByProduct = React.useCallback(() => {
    ...
    if (Math.abs(roundToTwo(processTransformationFactor) - 1) > 0.01) {
      toast.error(
        'La distribución 1:1 solo está disponible cuando entrada y salida del nodo tienen la misma cantidad total.'
      );
      return;
    }
    ...
    allProductIds.forEach((productId) => {
      ...
      const weightDifference = Math.abs(
        roundToTwo(outputGroup.totalWeight) - roundToTwo(sourceGroup.totalSourceWeight)
      );
      if (weightDifference > 0.01) {
        mismatchMessages.push(
          'No coinciden los pesos entre entradas y salidas del mismo producto.'
        );
      }
    });
    ...
    if (mismatchMessages.length > 0) {
      toast.error(
        'La distribución 1:1 requiere que entradas y salidas coincidan en productos y cantidades.'
      );
      return;
    }
    ...
    validRows.forEach((row) => {
      ...
      const totalMatchingWeight = matchingSources.reduce(
        (sum, source) => sum + source.totalWeight,
        0
      );
      ...
      updateRow(
        row.id,
        'source_priority',
        matchingSources.map((option) => option.key)
      );
      updateRow(row.id, 'sources', recalculateSourcesForRow(row, nextSources));
    });
  ...
  const addOutputsFromConsumptionsAndDistributeMatchingSources = React.useCallback(() => {
    const availableSources = sourceOptions.filter(
      (option) =>
        ...
        option.matched_product_id !== null &&
        option.matched_product_id !== undefined
    );
  ...
  const completeRowWithPriority = React.useCallback(
    (row) => {
      const outputWeight = parseFloat(row.weight_kg || 0) || 0;
      ...
      const currentTransformedWeight = currentSources.reduce(
        (sum, source) =>
          sum +
          getAdjustedOutputWeightFromSource(parseFloat(source.contributed_weight_kg || 0) || 0),
        0
      );
      ...
      orderedKeys.forEach((sourceKey) => {
        ...
        const requiredSourceWeight = getSourceWeightFromAdjustedOutputWeight(
          remainingTransformedWeight
        );
        const sourceWeightToApply = roundToTwo(
          Math.min(availableSourceWeight, requiredSourceWeight)
        );
        if (sourceWeightToApply <= 0) return;

        if (existingIndex >= 0) {
          const currentWeight =
            parseFloat(nextSources[existingIndex].contributed_weight_kg || 0) || 0;
  ...
  const moveGlobalOutputPriorityToTop = React.useCallback((rowId) => {
  ...
  const moveGlobalSourcePriorityToTop = React.useCallback((sourceKey) => {
    setGlobalSourcePriorityKeys((prev) => {
      const key = String(sourceKey || '');
      const oldIndex = prev.indexOf(key);
      if (oldIndex <= 0) return prev;
      return arrayMove(prev, oldIndex, 0);
    });
  }, []);
  ...
  const autoAssignSourcesByDoublePriority = React.useCallback(() => {
    ...
    orderedRows.forEach((row) => {
      ...
      globalSourcePriorityKeys.forEach((sourceKey) => {
        ...
        const requiredSourceWeight =
          getSourceWeightFromAdjustedOutputWeight(remainingAdjustedWeight);
        const sourceWeightToApply = roundToTwo(
          Math.min(remainingSourceWeight, requiredSourceWeight)
        );
        if (sourceWeightToApply <= 0) return;
    ...
    () =>
      globalOutputPriorityIds
        ...
        .map((row) => ({
          ...
          label:
            products.find(
              (product) => String(product.value ?? product.id) === String(row.product_id)
            )?.label || `Salida ${row.id}`,
  ...
  const availableProductsDialog = (
    <Dialog
      open={availableProductsDialogOpen}
      onOpenChange={(open) => {
    ...
    >
      <DialogContent size="4xl" className="max-h-[90vh]">
        <DialogHeader>
          ...
          <DialogDescription>
            Selecciona los productos que deseas agregar automáticamente. Estos productos tienen
            cajas y pesos registrados en ventas, stock y reprocesados.
          </DialogDescription>
        </DialogHeader>
        ...
        {loadingAvailableProducts ? (
          ...
          <div className="space-y-4">
              ...
              <Table>
                    ...
                    <TableHead className="w-[50px]">
                      <Checkbox
                        ...
                        onCheckedChange={(checked) => {
                          if (checked) {
                            setSelectedProducts(
                              new Set(availableProducts.map((p) => p.product.id))
                            );
                          } else {
                            setSelectedProducts(new Set());
                          }
                ...
                <TableBody>
                  {availableProducts.map((item) => {
                    ...
                    return (
                      <TableRow key={item.product.id} className={isSelected ? 'bg-muted/50' : ''}>
                        <TableCell>
                          <Checkbox
                            checked={isSelected}
                        ...
                        <TableCell className="text-right">{item.totalBoxes || 0}</TableCell>
                        ...
                        <TableCell>
                          <div className="text-muted-foreground flex flex-col gap-1 text-xs">
                            {item.sources?.sales &&
                              (item.sources.sales.boxes > 0 || item.sources.sales.weight > 0) && (
                                <span>
                                  Ventas: {item.sources.sales.boxes} cajas,{' '}
                                  {formatWeight(item.sources.sales.weight)}
                                </span>
                              )}
                            {item.sources?.stock &&
                              (item.sources.stock.boxes > 0 || item.sources.stock.weight > 0) && (
                                <span>
                                  Stock: {item.sources.stock.boxes} cajas,{' '}
                                  {formatWeight(item.sources.stock.weight)}
                                </span>
                              )}
                            {item.sources?.reprocessed &&
                              (item.sources.reprocessed.boxes > 0 ||
                                item.sources.reprocessed.weight > 0) && (
                                <span>
                                  Reprocesados: {item.sources.reprocessed.boxes} cajas,{' '}
                                  {formatWeight(item.sources.reprocessed.weight)}
                                </span>
                              )}
                          </div>
                        </TableCell>
            ...
            <div className="flex justify-end gap-2 border-t pt-2">
              <Button variant="outline" onClick={() => setAvailableProductsDialogOpen(false)}>
                Cancelar
              </Button>
              <Button
              ...
              >
                <Plus />
                Agregar {selectedProducts.size > 0 ? `${selectedProducts.size} ` : ''}Producto
  ...
  const mainContent = (
    <>
      ...
      {!hideTitle && !renderInCard && (
        <div className="flex items-center justify-between">
          <div>
            ...
            <p className="text-muted-foreground text-sm">Productos producidos en este proceso</p>
          </div>
        </div>
      )}
      {!renderInCard && (
      ...
      <div className={cn(renderInCard && outputs.length === 0 && 'flex min-h-0 flex-1 flex-col')}>
        {outputs.length === 0 ? (
          <div
            className={cn(
              'bg-muted/30 rounded-lg border-2 border-dashed text-center',
              renderInCard
    ...
    >
      <DialogContent size="4xl" className="max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Ordena las fuentes para completar esta salida al 100%. Se consumirán en este orden y
            siempre respetando la disponibilidad global restante.
          </DialogDescription>
        </DialogHeader>
              ...
              <div className="flex flex-wrap items-start justify-between gap-3">
                <div>
                  <p className="text-foreground text-sm font-semibold">
                    {products.find(
                      (product) =>
                        String(product.value ?? product.id) === String(priorityDialogRow.product_id)
                    )?.label || 'Salida seleccionada'}
                  </p>
                  <p className="text-muted-foreground text-xs">
                ...
                <div className="text-right">
                  <p className="text-muted-foreground text-xs">Cobertura actual</p>
                  <p className="text-foreground text-sm font-semibold">
                    {formatDecimal(
                      (priorityDialogRow.sources || []).reduce(
                        (sum, source) => sum + (parseFloat(source.contribution_percentage) || 0),
                        0
                      )
                    )}
                    %
              ...
              <div className="p-4">
                <DndContext
                  ...
                  onDragEnd={({ active, over }) => {
                    ...
                    setRowSourcePriority(
                      priorityDialogRow.id,
                      arrayMove(orderedKeys, oldIndex, newIndex)
                    );
                  }}
                >
                  <SortableContext
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setPriorityDialogRowId(null)}>
            Cancelar
          </Button>
          <Button
            onClick={() => {
    ...
    >
      <DialogContent size="4xl" className="max-h-[90vh] overflow-hidden">
        <DialogHeader>
          ...
          <DialogDescription>
            Primero se atenderán las salidas en el orden que elijas y, dentro de cada una, se
            consumirán las fuentes según su propio orden. Esta acción sobrescribe las fuentes de las
            salidas válidas.
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="grid gap-4 lg:grid-cols-2">
          <div className="space-y-3">
            <div className="bg-muted/20 rounded-lg border p-4">
              <p className="text-foreground text-sm font-semibold">
                1. Prioridad de productos de salida
              </p>
                    ...
                    <div className="grid gap-2">
                      {globalOutputPriorityItems.map((item) => (
                        <SortableOutputPriorityItem
                          ...
                          onClick={moveGlobalOutputPriorityToTop}
                        />
                      ))}
                    </div>
                  </SortableContext>
          ...
          <div className="space-y-3">
            <div className="bg-muted/20 rounded-lg border p-4">
              <p className="text-foreground text-sm font-semibold">2. Prioridad de fuentes</p>
              <p className="text-muted-foreground mt-1 text-xs">
                      ...
                      {globalSourcePriorityKeys.map((priorityKey) => {
                        ...
                        return (
                          <SortablePriorityItem
                            ...
                            onClick={() => moveGlobalSourcePriorityToTop(priorityKey)}
                          />
                        );
                      })}
                    </div>
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setGlobalPriorityDialogOpen(false)}>
            Cancelar
          </Button>
          <Button
  ...
  const globalSourcesSummaryContent =
    globalSourceSummary.length > 0 ? (
      <div className="space-y-4">
        <div className="bg-muted/20 rounded-lg border p-4">
          <div className="flex flex-wrap items-start justify-between gap-3">
            <div>
            ...
            <div className="flex flex-wrap gap-2">
              ...
              <Badge variant="outline">
                {formatWeight(
                  globalSourceSummary.reduce(
                    (sum, source) => sum + (source.assignedSourceWeight || 0),
                    0
                  )
                )}{' '}
                usadas
        ...
        <div className="grid min-h-0 gap-4 xl:grid-cols-[minmax(0,0.9fr)_minmax(0,1.1fr)]">
          <div className="bg-background min-h-0 rounded-lg border">
            <div className="border-b px-4 py-3">
              <h5 className="text-foreground text-sm font-semibold">Uso por fuente</h5>
              <p className="text-muted-foreground mt-1 text-xs">
            ...
            <div className="p-4">
              <Card className="overflow-visible border-0 shadow-none">
                ...
                <CardFooter className="flex-col items-start gap-2 px-0 pt-3 pb-3 text-sm">
                  <div className="min-w-0 leading-snug font-medium">
                    {formatWeight(
                      globalSourceSummary.reduce(
                        (sum, source) => sum + (source.assignedSourceWeight || 0),
                        0
                    ...
                    {formatWeight(
                      globalSourceSummary.reduce(
                        (sum, source) => sum + (source.totalSourceWeight || 0),
                        0
                      )
                    )}
                  </div>
                  <div className="text-muted-foreground min-w-0 leading-snug text-pretty">
                    A la derecha tienes el detalle agrupado por origen con su disponibilidad y
                    porcentaje de uso.
                  </div>
                </CardFooter>
              </Card>
          ...
          <div className="min-h-0 space-y-4">
            {[
              {
                key: 'stock',
                title: 'Fuentes desde stock',
                items: groupedGlobalSourceSummary.stock,
              },
              {
                key: 'parent',
                title: 'Fuentes desde proceso padre',
                items: groupedGlobalSourceSummary.parent,
              },
            ].map((group) =>
              group.items.length > 0 ? (
                <div key={group.key} className="bg-background overflow-hidden rounded-lg border">
                  <div className="flex items-center justify-between gap-3 border-b px-4 py-3">
                    <div>
                      <h5 className="text-foreground text-sm font-semibold">{group.title}</h5>
                      <p className="text-muted-foreground mt-1 text-xs">
      ...
      <div className="bg-muted/10 rounded-lg border border-dashed p-8 text-center">
        <p className="text-foreground text-sm font-medium">No hay fuentes globales disponibles</p>
        <p className="text-muted-foreground mt-1 text-xs">
        ...
        <div className="relative isolate z-0 flex min-h-0 min-w-0 flex-1 basis-0 flex-col overflow-hidden">
          {manageDialogBusy ? (
            <div
              ...
              aria-live="polite"
              aria-busy="true"
              className="dark:bg-background absolute inset-0 z-100 flex flex-col items-center justify-center bg-white"
            >
          ...
          >
              ...
              <div className="flex h-full min-h-0 flex-col gap-4">
                <div className="flex shrink-0 flex-wrap items-center justify-between gap-3">
                  ...
                  <div className="flex flex-wrap items-center justify-end gap-2">
                      ...
                      <DropdownMenuContent align="end" className="w-80">
                        ...
                        <DropdownMenuItem onClick={handleOpenAvailableProductsDialog}>
                          ...
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-muted-foreground text-xs">
                              Añade productos con cajas y pesos registrados en ventas, stock y
                              reprocesados.
                            </span>
                          </div>
                        </DropdownMenuItem>
                        ...
                        >
                          ...
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-muted-foreground text-xs">
                              Añade líneas de salida desde consumos del padre o materia prima desde
                              stock.
                            </span>
                          </div>
                        </DropdownMenuItem>
                        ...
                        >
                          ...
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-muted-foreground text-xs">
                              Crea las salidas faltantes desde los consumos y les asigna sus fuentes
                              del mismo producto.
                            </span>
                          </div>
                        </DropdownMenuItem>
                        ...
                        <DropdownMenuItem
                          ...
                          disabled={
                            allRows.filter((row) => parseFloat(row.weight_kg || 0) > 0).length ===
                              0 || sourceOptions.length === 0
                          }
                        >
                          <Sparkles className="mr-2 h-4 w-4" />
                          <div className="flex flex-col gap-0.5">
                        ...
                        <DropdownMenuItem
                          ...
                          disabled={
                            allRows.filter(
                              (row) => row.product_id && parseFloat(row.weight_kg || 0) > 0
                            ).length === 0 || sourceOptions.length === 0
                          }
                        >
                          <Sparkles className="mr-2 h-4 w-4" />
                          <div className="flex flex-col gap-0.5">
                            ...
                            <span className="text-muted-foreground text-xs">
                              Asigna a cada salida solo las fuentes del mismo producto, priorizando
                              una correspondencia 1:1.
                            </span>
                          </div>
                        </DropdownMenuItem>
                        <DropdownMenuItem
                    ...
                    <Button onClick={addNewRow} variant="default" size="sm">
                ...
                <ScrollArea className="min-h-0 flex-1 rounded-md border">
                  <Table>
                    ...
                    <TableBody>
                      {allRows.map((row) => {
                        const avgWeight =
                          row.boxes && parseFloat(row.boxes) > 0 && row.weight_kg
                            ? formatDecimal(parseFloat(row.weight_kg) / parseFloat(row.boxes))
                            : '0,00';
                        const sourcesTotalPercentage = Array.isArray(row.sources)
                          ? row.sources.reduce(
                              (sum, source) =>
                                sum + (parseFloat(source.contribution_percentage) || 0),
                              0
                            )
                          : 0;
                        const sourcesFullyConfigured =
                        ...
                        const isValid =
                          row.product_id &&
                          row.boxes &&
                          parseFloat(row.boxes) > 0 &&
                          row.weight_kg &&
                          parseFloat(row.weight_kg) > 0;

                        return (
                          <React.Fragment key={row.id}>
                            <TableRow
                              className={
                                !isValid && (row.product_id || row.boxes || row.weight_kg)
                                  ? 'bg-muted/50'
                                  : ''
                              }
                            >
                              <TableCell>
                                <div
                                  className={!row.product_id ? '[&_button]:border-destructive' : ''}
                                >
                                  <Combobox
                                    options={products}
                              ...
                              <TableRow className="bg-muted/40">
                                <TableCell colSpan={showBoxes ? 6 : 5} className="align-top">
                                  <div className="space-y-4">
                                    <div className="flex flex-wrap items-center justify-between gap-2">
                                      <div>
                                        <p className="text-foreground text-xs font-semibold">
                                          Orígenes de esta salida
                                        </p>
                                        <p className="text-muted-foreground text-xs">
                                          Establece desde qué stock o salidas del proceso padre
                                          deriva el peso de esta salida y reparte kilos y
                                          porcentajes hasta el 100 %.
                                        </p>
                                      </div>
                                    ...
                                    Array.isArray(row.sources) &&
                                    row.sources.length > 0 ? (
                                      <div className="bg-card space-y-0 overflow-hidden rounded-md border">
                                        <Table>
                                          <TableHeader>
                                            <TableRow className="hover:bg-transparent">
                                              <TableHead className="text-muted-foreground min-w-[220px]">
                                                Origen
                                              </TableHead>
                                              <TableHead className="text-muted-foreground w-[128px]">
                                                Peso fuente (kg)
                                              </TableHead>
                                              <TableHead className="text-muted-foreground w-[128px]">
                                                Peso transformado (kg)
                                              </TableHead>
                                              <TableHead className="text-muted-foreground w-[96px]">
                                                %
                                              </TableHead>
                                              ...
                                              const currentSourceValue =
                                                source.source_type === 'stock_product'
                                                  ...
                                                  : source.source_type === 'parent_output'
                                                    ...
                                                    : undefined;
                                              const sourceAvailableWeight =
                                                getAvailableSourceWeightForLine(
                                                  row,
                                                  sourceIndex,
                                                  currentSourceKey
                                                );
                                              const currentSourceWeight =
                                                parseFloat(source.contributed_weight_kg || 0) || 0;
                                              const additionalSourceWeight = roundToTwo(
                                                Math.max(
                                                  0,
                                                  sourceAvailableWeight - currentSourceWeight
                                                )
                                              );
                                              const currentAdjustedWeight =
                                                getAdjustedOutputWeightFromSource(
                                                  currentSourceWeight
                                                );
                                                ...
                                                <TableRow key={sourceIndex}>
                                                  <TableCell className="align-top">
                                                    <div className="flex flex-col gap-1.5">
                                                      ...
                                                      {currentSourceKey ? (
                                                        <p className="text-muted-foreground text-xs">
                                                          Disponible:{' '}
                                                          {formatWeight(additionalSourceWeight)}
                                                        </p>
                                                      ) : null}
                                                    </div>
                                                  </TableCell>
                                                  <TableCell>
                                                    <Input
                                                      ...
                                                      onChange={(e) => {
                                                        ...
                                                        const outputWeight =
                                                          parseFloat(row.weight_kg || 0) || 0;
                                                        const adjustedOutputWeight =
                                                          getAdjustedOutputWeightFromSource(weight);
                                                        updated[sourceIndex] = {
                                                          ...updated[sourceIndex],
                                                          contributed_weight_kg:
                                                            e.target.value === ''
                                                  ...
                                                  <TableCell>
                                                    <Input
                                                      ...
                                                      onChange={(e) => {
                                                        ...
                                                        const outputWeight =
                                                          parseFloat(row.weight_kg || 0) || 0;
                                                        let calculatedAdjustedOutputWeight =
                                                          outputWeight > 0
                                                            ? (percentage / 100) * outputWeight
                                                            : 0;
                                                        let calculatedWeightFromOutput =
                                                          getSourceWeightFromAdjustedOutputWeight(
                                                            calculatedAdjustedOutputWeight
                                                          );
                                                        if (
                                                          calculatedWeightFromOutput >
                                                          sourceAvailableWeight
                                                        ) {
                                                          calculatedWeightFromOutput =
                                                            sourceAvailableWeight;
                                                          calculatedAdjustedOutputWeight =
                                                        ...
                                                        updated[sourceIndex] = {
                                                          ...
                                                          contributed_weight_kg: roundToTwo(
                                                            calculatedWeightFromOutput
                                                          ),
                                                        };
                                                        updateRow(row.id, 'sources', updated);
                                                  ...
                                                  <TableCell>
                                                    <Button
                                                      ...
                                                      size="icon"
                                                      onClick={() => {
                                                        const updated = row.sources.filter(
                                                          (_, i) => i !== sourceIndex
                                                        );
                                                        updateRow(row.id, 'sources', updated);
                                                      }}
                                        ...
                                        {(() => {
                                          const totalPercentage = row.sources.reduce(
                                            (sum, s) =>
                                              sum + (parseFloat(s.contribution_percentage) || 0),
                                            0
                                          );
                                          const isComplete = Math.abs(totalPercentage - 100) < 0.01;
                                          const progressValue = Math.max(
                                            0,
                                            Math.min(totalPercentage, 100)
                                          );
                                          return (
                                            <div className="bg-muted/30 space-y-2 border-t px-4 py-3">
                                              <div className="flex items-center justify-between gap-3 text-sm">
                                                <span className="text-muted-foreground font-medium">
                                                  Total fuentes
                                                </span>
                                                <span
                                                  className={`font-medium tabular-nums ${isComplete ? 'text-green-600' : 'text-amber-600'}`}
                                                >
                                      ...
                                      <div className="bg-background/80 text-muted-foreground rounded-md border border-dashed px-3 py-4 text-xs">
            ...
            >
              <ScrollArea className="min-h-0 flex-1 rounded-md border">
                <div className="p-4">{globalSourcesSummaryContent}</div>
              </ScrollArea>
            </TabsContent>
          </Tabs>
        </div>
  ...
  if (renderInCard) {
    return (
      <>
        ...
        {breakdownDialog}
        {deleteConfirmDialog}
        <Card className={cn(outputs.length === 0 ? 'min-h-[min(22rem,55vh)]' : 'h-fit')}>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
          ...
          <CardContent className={cn(outputs.length === 0 && 'flex min-h-0 flex-1 flex-col')}>
            {mainContent}
          </CardContent>
        </Card>
      </>
