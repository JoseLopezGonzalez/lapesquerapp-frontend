 ...
 */
export default function ProductionCostsManager({ productionRecordId = null, productionId = null }) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const submitData = {
        ...
        cost_catalog_id:
          useCatalog && formData.cost_catalog_id ? parseInt(formData.cost_catalog_id) : null,
  ...
  return (
    <Card className={cn(showCostsEmpty && 'min-h-[min(22rem,55vh)]')}>
      <CardHeader className="flex flex-row items-center justify-between">
        ...
        <Dialog open={dialogOpen} onOpenChange={setDialogOpen}>
          ...
          <DialogContent size="2xl" className="max-h-[90vh] overflow-y-auto">
            <DialogHeader>
              <DialogTitle>{editingCost ? 'Editar Coste' : 'Nuevo Coste'}</DialogTitle>
              <DialogDescription>
                {productionRecordId
                ...
                <div className="space-y-2">
                  ...
                  <Select onValueChange={handleCatalogSelect} value={formData.cost_catalog_id}>
                    <SelectTrigger loading={catalogLoading}>
                      <SelectValue
                        placeholder="Selecciona un coste del catálogo"
      ...
      <CardContent className={cn(showCostsEmpty && 'flex min-h-0 flex-1 flex-col')}>
        {loading && costs.length === 0 ? (
          <div className="py-8 text-center">Cargando...</div>
        ) : costs.length === 0 ? (
          <div className="flex min-h-0 flex-1 flex-col">
            <EmptyState
              icon={<Calculator className="text-primary h-12 w-12" strokeWidth={1.5} />}
              ...
              className="bg-muted/30 h-full min-h-0 flex-1 rounded-lg border border-dashed"
            />
          </div>
        ) : (
          <Table>
