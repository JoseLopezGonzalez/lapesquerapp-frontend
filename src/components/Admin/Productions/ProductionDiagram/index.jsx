...
function FlowContent({
  ...
  const hasProcessTreeData =
    processTree && processTree.processNodes && processTree.processNodes.length > 0;
    ...
    return (
      <div className="flex h-[600px] items-center justify-center">
        <EmptyState
          ...
          description={
            isFiltered
              ? 'No hay cajas expedidas de este lote asociadas al cliente o pedido seleccionado.'
              : 'Crea procesos de producción para visualizar el diagrama'
          }
        />
      </div>
    );
  ...
  return (
    <div className="w-full">
      ...
      <div className="bg-background h-[600px] w-full rounded-lg border">
        ...
        >
          <Controls className="bg-background rounded-md border shadow-sm" />
          <Background variant="dots" gap={16} size={1} color="hsl(var(--muted))" />
        </ReactFlow>
      </div>
    </div>
  );
...
export default function ProductionDiagram({
  processTree,
  productionId,
  loading = false,
