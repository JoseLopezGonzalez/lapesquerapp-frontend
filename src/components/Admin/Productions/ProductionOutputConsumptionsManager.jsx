...
const ProductionOutputConsumptionsManager = ({
  productionRecordId,
  initialConsumptions: initialConsumptionsProp = [],
    ...
    >
      <DialogContent size="2xl">
        ...
        {loadingAvailableOutputs ? (
          ...
          <div className="flex items-center justify-center py-8">
            <EmptyState
              icon={<ArrowDown className="text-primary h-12 w-12" strokeWidth={1.5} />}
            ...
            <div>
              ...
              >
                <SelectTrigger loading={loadingAvailableOutputs}>
                  <SelectValue
                    placeholder="Selecciona un output"
                    loading={loadingAvailableOutputs}
                  />
                </SelectTrigger>
              ...
              {formData.production_output_id &&
                (() => {
                  ...
                  const originalWeight = existingConsumption
                    ? parseFloat(
                        existingConsumption.consumedWeightKg ||
                          existingConsumption.consumed_weight_kg ||
                          0
                      )
                    : 0;
                  ...
                  const consumptionPercentage =
                    totalWeight > 0 ? (consumedWeight / totalWeight) * 100 : 0;
            ...
            <div className="grid grid-cols-2 gap-4">
              <div>
                ...
                <Input
                  ...
                  max={
                    formData.production_output_id
                      ? (() => {
                          ...
                          const originalWeight = existingConsumption
                            ? getConsumedWeight(existingConsumption)
                            : 0;
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, consumed_weight_kg: e.target.value }))
                  }
                  placeholder="0.00"
                />
              </div>
              <div>
                ...
                <Input
                  ...
                  max={
                    formData.production_output_id
                      ? (() => {
                          ...
                          const originalBoxes = existingConsumption
                            ? getConsumedBoxes(existingConsumption)
                            : 0;
                  ...
                  onChange={(e) =>
                    setFormData((prev) => ({ ...prev, consumed_boxes: e.target.value }))
                  }
                  placeholder="0"
                />
              </div>
    ...
    <>
      ...
      {!renderInCard && (
        <div className={`flex items-center ${hideTitle ? 'justify-end' : 'justify-between'}`}>
          ...
          >
            <DialogTrigger asChild>{headerButton}</DialogTrigger>
      ...
      <div
        className={cn(renderInCard && isConsumptionsEmptyView && 'flex min-h-0 flex-1 flex-col')}
      >
        {!hasParent ? (
          <div
            className={cn(
          ...
          >
            <EmptyState
              icon={<ArrowDown className="text-muted-foreground h-12 w-12" strokeWidth={1.5} />}
                ...
                <TableBody>
                  {consumptions.map((consumption) => (
                    <TableRow key={consumption.id}>
                      ...
                      <TableCell>{formatWeight(getConsumedWeight(consumption))}</TableCell>
    ...
    >
      <DialogContent size="5xl" className="max-h-[90vh] overflow-y-auto">
        <DialogHeader>
          ...
          <DialogDescription>
            Agrega, edita o elimina múltiples consumos de productos del proceso anterior de forma
            rápida
          </DialogDescription>
        </DialogHeader>
        ...
        <div className="relative">
          {manageDialogBusy ? (
            <div
              ...
              aria-live="polite"
              aria-busy="true"
              className="dark:bg-background absolute inset-0 z-50 flex flex-col items-center justify-center bg-white"
            >
          ...
          <div className="flex flex-col gap-2">
                ...
                <TooltipProvider>
                  <Tooltip>
                    ...
                    <TooltipContent className="max-w-xs">
                      <p className="text-sm">
                        Añade automáticamente todas las líneas de consumo con los productos y
                        disponibilidades del proceso padre. Solo se añaden los outputs que tienen
                        disponibilidad y que no están ya en la lista.
                      </p>
                    </TooltipContent>
                  </Tooltip>
                </TooltipProvider>
                <Button onClick={addNewConsumptionRow} variant="outline" size="sm">
            ...
            <ScrollArea className="h-[min(500px,55vh)] rounded-md border">
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {getAllConsumptionRows().map((row) => {
                    ...
                    const rowOutputId = row.production_output_id
                      ? String(row.production_output_id).trim()
                      : null;
                    const selectedOutput = rowOutputId
                      ? availableOutputs.find((o) => {
                          const outputId = o.output?.id;
                          if (!outputId) return false;
                          // Comparar tanto como string como number para evitar problemas de tipo
                          return (
                            String(outputId) === rowOutputId ||
                            Number(outputId) === Number(rowOutputId)
                          );
                        })
                      : null;
                    ...
                    const baseAvailableWeight = selectedOutput
                      ? parseFloat(selectedOutput.availableWeight || 0)
                      : 0;
                    const adjustedAvailableWeight = baseAvailableWeight + originalWeight;

                    const baseAvailableBoxes = selectedOutput
                      ? selectedOutput.availableBoxes || 0
                      : 0;
                    const adjustedAvailableBoxes = baseAvailableBoxes + originalBoxes;

                    const totalWeight = selectedOutput
                      ? parseFloat(selectedOutput.totalWeight || 0)
                      : 0;
                    const consumedWeight = selectedOutput
                      ? parseFloat(selectedOutput.consumedWeight || 0)
                      : 0;
                    const rowWeight = parseFloat(row.consumed_weight_kg || 0);

                    const isValid =
                      row.production_output_id && row.consumed_weight_kg && rowWeight > 0;
                    ...
                    return (
                      <React.Fragment key={row.id}>
                        <TableRow
                          className={
                            !isValid && (row.production_output_id || row.consumed_weight_kg)
                              ? 'bg-muted/50'
                              : ''
                          }
                        >
                          <TableCell className="align-top">
                            <div className="space-y-2 py-1">
                              <Select
                                ...
                                onValueChange={(value) => {
                                  ...
                                  const output = availableOutputs.find((o) => {
                                    ...
                                    return (
                                      outputId &&
                                      (String(outputId) === String(value) ||
                                        Number(outputId) === Number(value))
                                    );
                                  });
                                  if (output) {
                                    // Si es edición, ajustar disponibilidad sumando el peso original
                                    const isEditing =
                                      !row.isNew && row.originalWeight !== undefined;
                                    const originalWeight = isEditing ? row.originalWeight || 0 : 0;
                                    const originalBoxes = isEditing ? row.originalBoxes || 0 : 0;
                                    // availableWeight viene directamente en el objeto, no en output
                                    const adjustedAvailable =
                                      parseFloat(output.availableWeight || 0) + originalWeight;
                                    const adjustedBoxes =
                                      parseInt(output.availableBoxes || 0) + originalBoxes;
                                    ...
                                    updateConsumptionRow(
                                      row.id,
                                      'consumed_weight_kg',
                                      formatNumber(adjustedAvailable)
                                    );
                                    if (adjustedBoxes > 0) {
                                      updateConsumptionRow(
                                        row.id,
                                        'consumed_boxes',
                                        adjustedBoxes.toString()
                                      );
                                    }
                                    if (productName) {
                              ...
                              >
                                <SelectTrigger
                                  className={`h-9 ${!row.production_output_id ? 'border-destructive' : ''}`}
                                  loading={loadingAvailableOutputs}
                                >
                                  <SelectValue
                                    placeholder="Seleccionar output"
                                    loading={loadingAvailableOutputs}
                                  />
                                </SelectTrigger>
                                <SelectContent loading={loadingAvailableOutputs}>
                                  {availableOutputs.length === 0 ? (
                                    ...
                                    availableOutputs.map((available) => {
                                      ...
                                      const isUsed = getAllConsumptionRows().some(
                                        (r) =>
                                          r.id !== row.id &&
                              ...
                              row.production_output_id.trim() !== '' ? (
                                selectedOutput ? (
                                  // Panel de disponibilidad cuando se encuentra el output
                                  <div
                                    className="bg-muted/30 mt-2 rounded-md border p-2"
                                    key={`panel-${row.id}-${row.production_output_id}`}
                                  >
                                    <div className="flex items-center justify-between gap-2">
                                      <div className="flex min-w-0 flex-1 items-center gap-2">
                                        <div className="bg-secondary h-2 w-full overflow-hidden rounded-full">
                                        ...
                                        <span className="text-muted-foreground text-xs whitespace-nowrap">
                                          {formatNumber(consumedWeight)}/{formatNumber(totalWeight)}
                                          kg
                                        </span>
                                      </div>
                                      <div className="flex items-center gap-1.5">
                                  ...
                                  <div className="mt-2 rounded-md border border-yellow-200 bg-yellow-50 p-2 dark:border-yellow-800 dark:bg-yellow-900/20">
                                    <p className="text-xs text-yellow-800 dark:text-yellow-200">
                                      ⚠ Output no encontrado (ID: {row.production_output_id}).{' '}
                                      {availableOutputs.length === 0
                                        ? 'No hay outputs disponibles cargados.'
                                        : `Hay ${availableOutputs.length} outputs disponibles.`}
                                    </p>
                                  </div>
                                )
                              ) : null}
                          ...
                          <TableCell className="align-top">
                            <div className="py-1">
                              <Input
                                ...
                                onChange={(e) =>
                                  updateConsumptionRow(row.id, 'consumed_weight_kg', e.target.value)
                                }
                                placeholder="0.00"
                            ...
                            <TableCell className="align-top">
                              <div className="py-1">
                                <Input
                                  ...
                                  onChange={(e) =>
                                    updateConsumptionRow(row.id, 'consumed_boxes', e.target.value)
                                  }
                                  placeholder="0"
                    ...
                    <TableRow>
                      <TableCell colSpan={4} className="p-0">
                        <div className="py-12">
                          <EmptyState
                            icon={
                              <ArrowDown className="text-primary h-12 w-12" strokeWidth={1.5} />
                            }
        ...
        <DialogFooter>
          <Button variant="outline" onClick={() => setManageDialogOpen(false)} disabled={savingAll}>
            Cancelar
          </Button>
          <Button
            ...
            disabled={
              savingAll ||
              getAllConsumptionRows().some((row) => {
                ...
                const hasWeight =
                  row.consumed_weight_kg && parseFloat(row.consumed_weight_kg || 0) > 0;
  ...
  if (renderInCard) {
    return (
      <>
        {manageDialog}
        {dialog}
        <Card className={cn(isConsumptionsEmptyView ? 'min-h-[min(22rem,55vh)]' : 'h-fit')}>
          <CardHeader>
            <div className="flex items-center justify-between">
              <div>
          ...
          <CardContent className={cn(isConsumptionsEmptyView && 'flex min-h-0 flex-1 flex-col')}>
            {mainContent}
          </CardContent>
        </Card>
      </>
