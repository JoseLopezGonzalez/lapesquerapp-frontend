  ...
  const generateExcelForA3erp = async () => {
    ...
    if (isVentaDirecta) {
      for (const [matricula, lines] of Object.entries(groupedByBarco)) {
        const barcoData = barcos.find(
          (a) => normalizeText(a.matricula) === normalizeText(matricula)
        );
        if (!barcoData) {
      ...
      <div className="grid gap-4 py-4">
        ...
        <div className="border-border bg-muted/30 flex items-start gap-3 rounded-md border p-3">
          ...
          <div className="grid gap-1.5 leading-none">
            <Label
              htmlFor="tasa-pesca-repercusion-asoc"
              className="cursor-pointer text-sm font-medium"
            >
              Repercutir tasa pesca fresca (T4) en gastos exportados
            </Label>
            <p className="text-muted-foreground text-xs">
              Desmarcar durante exención: sin Tarifa G-4 ni 3 % sostenibilidad; solo Gastos Lonja y
              Gestión cobros (1,5 % total sobre base).
            </p>
          </div>
        </div>
        <div className="flex flex-col gap-1">
          {isVentaDirecta && (
            <div className="flex w-fit items-center gap-1 rounded-md border border-white bg-white/30 p-1 px-2 text-white">
              <span className="text-xs">Albarán de Venta Directa</span>
            </div>
          )}

          {isSubasta && (
            <div className="flex w-fit items-center gap-1 rounded-md border border-white bg-white/30 p-1 px-2 text-white">
              <span className="text-xs">Albarán de Subasta</span>
            </div>
          )}
        </div>
