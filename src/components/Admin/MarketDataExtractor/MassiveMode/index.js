...
export default function MassiveMode() {
  ...
  const handleDocumentTypeChange = (documentId, documentType) => {
    setDocuments((prev) =>
      prev.map((doc) => (doc.id === documentId ? { ...doc, documentType } : doc))
    );
  };
  ...
  const handleRetryDocument = async (documentId) => {
    ...
    setDocuments((prev) =>
      prev.map((d) => (d.id === documentId ? { ...d, status: 'processing', error: null } : d))
    );
  ...
  const processedDocuments = documents.filter(
    (doc) => doc.status === 'success' || doc.status === 'processing'
  );
  ...
  return (
    <div className="bg-background flex h-full gap-4 overflow-hidden">
          ...
          <div className="flex items-center justify-between gap-2">
            ...
            <div className="flex flex-shrink-0 items-center gap-2">
              ...
              {pendingDocuments.length > 0 && (
                <Button size="sm" onClick={handleProcessAll} disabled={isProcessing}>
        ...
        <CardContent className="min-h-0 flex-1 overflow-y-auto p-4">
          {pendingDocuments.length > 0 ? (
            <div className="space-y-2">
              {pendingDocuments.map((doc) => (
                ...
                >
                  ...
                  {doc.error && (
                    <div className="flex items-start gap-2 rounded border border-red-300 bg-red-100 p-2 text-sm text-red-700">
            ...
            <div className="flex h-full items-center justify-center">
              <EmptyState
                icon={<Upload className="text-primary h-12 w-12" strokeWidth={1.5} />}
      ...
      <Card className="flex min-w-0 flex-1 flex-col overflow-hidden">
        ...
        {hasSuccessfulDocuments && (
          <div className="bg-card flex flex-shrink-0 gap-2 border-t px-4 py-3">
            <Button onClick={handleOpenExportDialog} className="flex-1">
              <Download className="mr-2 h-4 w-4" />
              Exportar Excel
            </Button>
            <Button onClick={handleOpenLinkDialog} variant="outline" className="flex-1">
