...
import {
  isoToDateTimeLocal,
  datetimeLocalToIsoWithZone,
} from '@/helpers/production/dateFormatters';
...
function prepareValidations(fields) {
  return fields.map((field) => {
    ...
    if (
      ...
      newField.validation.pattern.value = new RegExp(
        newField.validation.pattern.value.replace(/^\/|\/$/g, '')
      );
    }
    return newField;
  });
...
export default function EditEntityForm({ config, id: propId, onSuccess, onCancel }) {
  ...
  const { title, endpoint, method = 'PUT', successMessage, errorMessage } = config.editForm;
  ...
  const onSubmit = async (data) => {
    ...
    try {
      ...
      if (endpoint === 'customers' && entitySaved) {
        userMessage =
          'Los datos generales del cliente se guardaron, pero no se pudo actualizar la asignación operativa.';
      } else if (
        endpoint === 'customers' &&
        userMessage === (errorMessage || 'Error inesperado al guardar.')
      ) {
        userMessage =
  ...
  return (
    <div className="flex h-full flex-col">
      <ScrollArea className="h-full max-h-[70vh] w-full p-2">
        <form
          id="entity-form"
        ...
        >
          {preparedFields.map((field, index) => (
            ...
            >
              ...
              {renderField(field)}
              {errors[field.name] && (
                <p className="pt-1 text-xs text-red-400">* {errors[field.name].message}</p>
              )}
            </div>
          ))}
        </form>
        ...
        <Button
          ...
          title={
            Object.keys(errors).length > 0
              ? 'Hay errores. Pulsa para verlos junto a cada campo.'
              : undefined
          }
          aria-label={
            Object.keys(errors).length > 0 ? 'Hay errores: pulsa para ver los detalles' : undefined
          }
        >
          {isSubmitting ? (
            <>
