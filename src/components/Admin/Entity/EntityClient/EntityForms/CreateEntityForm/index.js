...
import { useEffect, useState, useCallback, useMemo } from 'react'; // Added useCallback
...
export default function CreateEntityForm({ config, onSuccess, onCancel }) {
  ...
  const fields = useMemo(
    () => config.createForm?.fields || config.fields || [],
    [config.createForm?.fields, config.fields]
  );
  ...
  const onSubmit = async (data) => {
    try {
      ...
      try {
        ...
        if (isCustomersEndpoint && createdId) {
          userErrorMessage =
            'El cliente se creó, pero no se pudo guardar su asignación operativa. Revisa el cliente recién creado e inténtalo de nuevo.';
        } else if (
          isCustomersEndpoint &&
          userErrorMessage === (errorMessage || 'Error al crear la entidad')
        ) {
