...
export default function EntityClient({ config }) {
  ...
  const isCatalogBlock =
    config.endpoint === 'transports' ||
    config.endpoint === 'incoterms' ||
    config.endpoint === 'salespeople';
  const isBlock5Clients =
    config.endpoint === 'customers' ||
    config.endpoint === 'payment-terms' ||
  ...
  const queryResult =
    config.endpoint === 'users'
      ...
      : config.endpoint === 'sessions'
        ? sessionsListResult
        : config.endpoint === 'transports'
          ? transportsListResult
          : config.endpoint === 'incoterms'
            ? incotermsListResult
            : config.endpoint === 'salespeople'
              ? salespeopleListResult
              : config.endpoint === 'customers'
                ? customersListResult
                : config.endpoint === 'suppliers'
                  ? suppliersListResult
                  : config.endpoint === 'payment-terms'
                    ? paymentTermsListResult
                    : config.endpoint === 'countries'
                      ? countriesListResult
                      : null;
  ...
  const fetchData = useCallback(
    async (page, currentFilters) => {
      ...
      try {
        ...
        const validPage =
          apiCurrentPage > apiTotalPages && apiTotalPages > 0
            ? apiTotalPages
            : apiTotalPages > 0
              ? apiCurrentPage
              : 1;
  ...
  const handleReport = async (reportOption) => {
    ...
    try {
      await notify.promise(
        downloadFile(url, fileName, type || 'pdf'),
        {
          loading: {
  ...
  const handleResendInvitation = useCallback(
    async (id) => {
      ...
      try {
        ...
        const errorMessage =
          err?.data?.userMessage || err?.message || 'Error al reenviar la invitación.';
        notify.error({ title: errorMessage });
      }
    },
  ...
  const handleCustomRowAction = useCallback(
    async (action, id, row) => {
      const entityService = getEntityService(config.endpoint);
      if (
        !entityService ||
        !action?.serviceMethod ||
        typeof entityService[action.serviceMethod] !== 'function'
      ) {
      ...
      let confirmationText =
        typeof action.confirmation === 'function' ? action.confirmation(row) : action.confirmation;
      if (typeof confirmationText === 'string' && confirmationText.includes('{{name}}')) {
        confirmationText = confirmationText.replace(
          /\{\{name\}\}/g,
          row?.name || 'este usuario externo'
        );
      }
