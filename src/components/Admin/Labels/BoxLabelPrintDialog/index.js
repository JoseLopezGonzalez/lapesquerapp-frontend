      ...
      <DialogContent className="overflow-hidden" style={{ maxWidth: `${maxDialogWidth}px` }}>
          ...
          <div className="space-y-6 px-2 pr-3">
            {isLoadingLabel && (
              ...
              >
                <div className="flex flex-col items-center justify-center gap-3">
                  ...
                  >
                    <SquareMousePointer className="text-primary h-6 w-6" />
                  </motion.div>
                  <motion.p
                    initial={{ opacity: 0 }}
                    animate={{ opacity: 1 }}
                ...
                <div className="mt-4 flex flex-col items-center gap-4">
                  <div className="bg-orange-200 px-4">
                    <div className="flex flex-col items-center gap-4">
                      <div className="h-20 w-full rounded-b-xl border border-t-0 bg-white shadow"></div>
                      <div
                        className={`flex-shrink-0 ${printInPairs ? 'flex flex-col gap-0' : ''}`}
                        style={
                          printInPairs ? { width: `${label?.format?.canvas?.width || 110}mm` } : {}
                        }
                      >
                        {printInPairs ? (
                          <>
                      ...
                      <div className="h-20 w-full rounded-t-xl border border-b-0 bg-white"></div>
                    </div>
                  </div>
                </div>
              </div>
                        ...
                        <div className="flex items-center space-x-2">
                          <Checkbox
                            ...
                            onCheckedChange={(checked) =>
                              handleOnChangeManualField(key, checked ? (meta?.content ?? '') : '')
                            }
                          />
                          <Label
                            htmlFor={`field-${key}`}
            ...
            {!isLoadingLabel && (
              <div className="flex items-center space-x-2">
                ...
                />
                <Label htmlFor="print-in-pairs" className="cursor-pointer text-sm font-normal">
                  ...
                  (() => {
                    ...
                    return pairs.map((pair, pairIndex) => (
                      <div
                        key={pairIndex}
                        className="page"
                        style={{ display: 'flex', flexDirection: 'column', gap: '0mm' }}
                      >
                        {pair.filter(Boolean).map((valuesElements, labelIndex) => (
                          <div key={labelIndex} style={{ width: '100%' }}>
                            <LabelRender label={label?.format} values={valuesElements} />
                          </div>
                        ))}
                        ...
                        {pair.filter(Boolean).length === 1 && (
                          <div
                            style={{
                              width: '100%',
                              height: `${label?.format?.canvas?.height || 90}mm`,
                            }}
                          ></div>
                        )}
                      </div>
        ...
        {!isLoadingLabel && (
          <DialogFooter className="pt-4">
            <Button onClick={handleOnClickPrintLabel} className="w-full">
