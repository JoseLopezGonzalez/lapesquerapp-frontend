...
export default function LabelRender({
  ...
  return (
    <div
      className={`relative bg-white text-black ${showPreviewBorder ? 'border-border rounded-lg border-2 shadow-lg' : ''}`}
      style={{
        width: `${width}mm`,
        height: `${height}mm`,
    ...
    >
      {elements.map((el) => {
        if (
          ['manualField', 'selectField', 'checkboxField', 'dateField'].includes(el.type) &&
          el.visibleOnLabel === false
        ) {
        ...
        return (
          <div
            key={el.id}
            className="hover:border-muted-foreground/30 absolute flex border border-transparent"
            style={{
              left: `${el.x}mm`,
              top: `${el.y}mm`,
              width: `${w}mm`,
