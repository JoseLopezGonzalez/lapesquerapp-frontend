...
export default function LabelEditor() {
  ...
  const getDateFieldPreview = (el, elementsList, visited = new Set()) => {
    ...
    if (mode === 'system' || mode === 'systemOffset')
      return formatDate(addDays(today, el.systemOffsetDays ?? 0));
  ...
  const hasDuplicateKey = useMemo(() => {
    ...
    return elements.some(
      (el) =>
        keyFieldTypes.includes(el.type) &&
        el.id !== activeElementState.id &&
        String(el.key || '').trim() === currentKey
    );
          ...
          <div className="flex h-full flex-1 flex-col pb-5">
            ...
            {selectedLabel && (
              <div className="flex h-full flex-col">
                ...
                >
                  <div className="flex h-full flex-1 items-center justify-center overflow-auto p-8">
                    <div className="flex h-full w-full items-center justify-center">
                      {zoom === 1 ? (
                        <div className="mt-4 flex flex-col items-center gap-4">
                          <div className="bg-orange-200 px-4">
                            <div className="flex flex-col items-center gap-4">
                              <div className="h-20 w-full rounded-b-xl border border-t-0 bg-white shadow" />
                              <LabelEditorPreview
                                canvasRef={canvasRef}
                                zoom={zoom}
                                canvasWidth={canvasWidth}
                              ...
                              />
                              <div className="h-20 w-full rounded-t-xl border border-b-0 bg-white" />
                            </div>
                          </div>
                        </div>
                      ) : (
          ...
          <DialogContent size="md">
            ...
            <div className="space-y-4 py-4">
              <div className="space-y-3">
                <div>
                  <h4 className="text-foreground mb-2 text-sm font-semibold">Movimiento</h4>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground text-sm">Mover elemento</span>
                      <div className="flex items-center gap-1">
                        <kbd className="text-foreground bg-muted border-border rounded border px-2 py-1 text-xs font-semibold">
                          ↑
                        </kbd>
                        <kbd className="text-foreground bg-muted border-border rounded border px-2 py-1 text-xs font-semibold">
                          ↓
                    ...
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground text-sm">Mover rápido (5mm)</span>
                      <div className="flex items-center gap-1">
                        <kbd className="text-foreground bg-muted border-border rounded border px-2 py-1 text-xs font-semibold">
                          Shift
                        </kbd>
                ...
                <Separator />
                <div>
                  <h4 className="text-foreground mb-2 text-sm font-semibold">Acciones</h4>
                  <div className="space-y-2">
                    <div className="flex items-center justify-between">
                      <span className="text-muted-foreground text-sm">Eliminar elemento</span>
                      <div className="flex items-center gap-1">
                        <kbd className="text-foreground bg-muted border-border rounded border px-2 py-1 text-xs font-semibold">
                          Delete
                        </kbd>
              ...
              <div className="bg-muted/50 border-muted rounded-lg border px-3 py-2">
                <p className="text-muted-foreground text-xs">
                  <strong>Nota:</strong> Los atajos solo funcionan cuando un elemento está
                  seleccionado y no estás escribiendo en un campo de texto.
                </p>
              </div>
            </div>
          </DialogContent>
