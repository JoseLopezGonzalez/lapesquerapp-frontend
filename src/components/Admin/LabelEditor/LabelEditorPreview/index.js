...
export default function LabelEditorPreview({
  ...
  return (
    <div
      ref={canvasRef}
      className={`relative bg-white ${isZoomed ? 'border-border/70 rounded-sm border-2 shadow-md' : 'border-border rounded-lg border-2 shadow-lg'}`}
      style={{
      ...
      {elements
        ...
        .map((element) => {
          ...
          return (
            <div
              key={element.id}
              className={`absolute flex cursor-move transition-colors ${
            ...
            >
              <LabelElement
                element={element}
                /* getFieldValue={getFieldValue} manualValues={manualValues} */ values={values}
              />
              ...
              {selectedElement === element.id && (
                <>
                  <div
                    ...
                    className="absolute -top-1 -left-1 h-2 w-2 cursor-nwse-resize rounded-full border-2 border-white bg-blue-600 shadow-sm"
                    style={{ zIndex: 10, transform: `scale(${1 / zoom})` }}
                  ></div>
                  <div
                    onMouseDown={(e) => handleResizeMouseDown(e, element.id, 'ne')}
                    className="absolute -top-1 -right-1 h-2 w-2 cursor-nesw-resize rounded-full border-2 border-white bg-blue-600 shadow-sm"
                    style={{ zIndex: 10, transform: `scale(${1 / zoom})` }}
                  ></div>
                  <div
                    onMouseDown={(e) => handleResizeMouseDown(e, element.id, 'sw')}
                    className="absolute -bottom-1 -left-1 h-2 w-2 cursor-nesw-resize rounded-full border-2 border-white bg-blue-600 shadow-sm"
                    style={{ zIndex: 10, transform: `scale(${1 / zoom})` }}
                  ></div>
                  <div
                    onMouseDown={(e) => handleResizeMouseDown(e, element.id, 'se')}
                    className="absolute -right-1 -bottom-1 h-2 w-2 cursor-nwse-resize rounded-full border-2 border-white bg-blue-600 shadow-sm"
                    style={{ zIndex: 10, transform: `scale(${1 / zoom})` }}
                  ></div>
                </>
              )}
