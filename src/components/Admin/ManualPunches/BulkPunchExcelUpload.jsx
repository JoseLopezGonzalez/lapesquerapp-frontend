...
export default function BulkPunchExcelUpload() {
  ...
  const findEmployeeByName = (name) => {
    ...
    let employee = employeeOptions.find((emp) => String(emp.label).toLowerCase() === nameLower);
    // Si no hay coincidencia exacta, buscar parcial
    if (!employee) {
      employee = employeeOptions.find(
        (emp) =>
      ...
      reader.onload = (e) => {
        try {
          ...
          const hasHeaders = firstRow.some((cell) => {
            ...
            return (
              cellLower.includes('empleado') ||
              cellLower.includes('employee') ||
              cellLower.includes('tipo') ||
              cellLower.includes('event') ||
              cellLower.includes('fecha') ||
              cellLower.includes('timestamp')
            );
          });
          ...
          for (let i = startIndex; i < jsonData.length; i++) {
            ...
            if (headers) {
              ...
              const employeeIdx = expectedHeaders.findIndex(
                (h) => h.includes('empleado') || h.includes('employee')
              );
              const eventTypeIdx = expectedHeaders.findIndex(
                (h) => h.includes('tipo') || h.includes('event')
              );
              const timestampIdx = expectedHeaders.findIndex(
                (h) => h.includes('fecha') || h.includes('timestamp') || h.includes('hora')
              );
            ...
            try {
              ...
              const isNumeric =
                typeof timestampOriginal === 'number' ||
              ...
              if (isNumeric) {
                ...
                const dateTimeMatch = timestampString.match(
        ...
        employeeOptions.length > 0
          ? [
              ...
              employeeOptions.length > 1
                ? employeeOptions[1]?.label
                : employeeOptions[0]?.label || 'Ejemplo Empleado 2',
              employeeOptions.length > 1
                ? employeeOptions[1]?.label
                : employeeOptions[0]?.label || 'Ejemplo Empleado 2',
            ]
  ...
  return (
    <Card className="flex h-full flex-col overflow-hidden">
      <CardHeader className="flex-shrink-0">
        ...
        <CardDescription>Importa múltiples fichajes desde un archivo Excel (.xlsx)</CardDescription>
      </CardHeader>
      <CardContent className="min-h-0 flex-1 space-y-6 overflow-y-auto">
        {/* Indicador de carga de empleados */}
        {loadingEmployees && (
          <div className="flex items-center gap-2 rounded-md border border-blue-200 bg-blue-50 p-3 dark:border-blue-800 dark:bg-blue-950/20">
        ...
        {fileName && (
          <div className="bg-muted flex items-center gap-2 rounded-md p-3">
            ...
            <span className="text-muted-foreground text-xs">
              ({parsedData.length}{' '}
              {parsedData.length === 1 ? 'fichaje parseado' : 'fichajes parseados'})
            </span>
          </div>
        )}
        ...
        {parsedData.length > 0 ? (
                      ...
                      <TableCell>
                        ...
                        >
                          {row.event_type_display ||
                            (row.event_type === 'IN' ? 'Entrada' : 'Salida')}
                        </span>
                      </TableCell>
                      <TableCell>
          ...
          <div className="rounded-md border border-dashed py-12 text-center">
            ...
            <p className="text-muted-foreground mb-2 text-sm">No hay archivo cargado</p>
            <p className="text-muted-foreground mb-4 text-xs">
              Selecciona un archivo Excel (.xlsx) o descarga la plantilla para ver el formato
              esperado
            </p>
        ...
        <div className="text-muted-foreground bg-muted space-y-1 rounded-md p-4 text-xs">
          <p className="mb-2 font-medium">Formato del Excel:</p>
