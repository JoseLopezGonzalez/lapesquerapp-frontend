...
export function TimelineEventDetail({ entry }) {
  ...
  switch (type) {
    ...
    case 'pallet_created_from_reception':
      return (
        <DetailPalletCreated d={details} fromReception={type === 'pallet_created_from_reception'} />
      );
    case 'pallet_updated':
      ...
      <CardContent className="pt-4 pb-4">
        <div className="grid gap-2 text-sm">
          ...
          <div className="text-muted-foreground flex flex-wrap gap-x-4 gap-y-1">
            ...
            {d.gs1128 != null && d.gs1128 !== '' && (
              <span className="font-mono text-xs">GS1: {d.gs1128}</span>
            )}
          </div>
          {(d.newBoxesCount != null || d.newTotalNetWeight != null) && (
            <p className="text-muted-foreground border-border/50 border-t pt-1 text-xs">
              Total palet: {d.newBoxesCount ?? '—'} cajas /{' '}
              {d.newTotalNetWeight != null ? formatDecimalWeight(d.newTotalNetWeight) : '—'}
            </p>
          )}
        </div>
      </CardContent>
        ...
        <div className="grid gap-2 text-sm">
          ...
          {changeKeys.length > 0 && (
            <div className="bg-muted/50 space-y-1 rounded-md border p-3">
              <p className="text-muted-foreground text-xs font-medium">Cambios</p>
              {changeKeys.map((key) => {
                const c = changes[key];
                if (!c || (c.from === undefined && c.to === undefined)) return null;
                return (
                  <p key={key} className="text-xs">
                    <span className="capitalize">{key}</span>: {String(c.from ?? '—')} →{' '}
                    {String(c.to ?? '—')}
                  </p>
                );
              })}
        ...
        {isAuto && (d.reason != null || d.usedBoxesCount != null) && (
          <p className="text-muted-foreground mt-2 text-xs">
            ...
            {d.usedBoxesCount != null && d.totalBoxesCount != null && (
              <span>
                {' '}
                · Cajas: {d.usedBoxesCount}/{d.totalBoxesCount}
              </span>
            )}
          </p>
        )}
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1 pt-0 text-sm">
        ...
        {previousValue && (
          <p className="text-muted-foreground text-xs">Anterior: {previousValue}</p>
        )}
      </CardContent>
    </Card>
  );
...
function DetailPosition({ d, isUnassigned }) {
  ...
  const storeValue = d.storeName != null ? d.storeName : d.storeId != null ? `#${d.storeId}` : null;
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1 pt-0 text-sm">
        ...
        {storeValue && <p className="text-muted-foreground text-xs">Almacén: {storeValue}</p>}
      </CardContent>
    </Card>
  );
}
...
function DetailOrder({ d, isUnlinked }) {
  ...
  const actionLabel = isUnlinked ? 'Desvinculado' : hasPrevious ? 'Modificado' : 'Vinculado';
  ...
  if (isUnlinked) {
    ...
    content = (
      <div className="flex items-stretch gap-3 text-sm">
        <div className="bg-muted/40 border-border/60 flex min-w-0 flex-1 flex-col rounded-md border px-2 py-1.5">
          <p className="text-muted-foreground line-clamp-6 break-words whitespace-pre-wrap">
            {fromRef}
          </p>
        </div>
        <span className="text-muted-foreground flex shrink-0 items-center py-1" aria-hidden>
          <ArrowRight className="h-4 w-4" />
        </span>
        <div className="bg-muted/40 border-border/60 flex min-w-0 flex-1 flex-col rounded-md border px-2 py-1.5">
          <p className="line-clamp-6 break-words whitespace-pre-wrap">{toRef}</p>
        </div>
      </div>
    );
  }
  ...
  return (
    <Card>
      ...
      <CardContent className="space-y-1 pt-0 text-sm">{content}</CardContent>
    </Card>
  );
}
...
function DetailBoxesTable({ boxes, variant, title }) {
  ...
  if (isUpdated) {
    const getVal = (box, key, side) => {
      ...
      if (c && (c.from !== undefined || c.to !== undefined))
        return side === 'from' ? (c.from ?? '—') : (c.to ?? '—');
      const v = box[key];
              ...
              {boxes.map((box, i) => (
                <TableRow key={box.boxId ?? i}>
                  <TableCell className="text-muted-foreground font-medium">
                    {getVal(box, 'productName', 'from') !== '—'
                      ? getVal(box, 'productName', 'from')
                      : (box.productName ??
                        (box.productId != null ? `Producto #${box.productId}` : '—'))}
                  </TableCell>
                  ...
                  <TableCell className="text-muted-foreground">
                    {fmtWeight(getVal(box, 'netWeight', 'from'))}
                  </TableCell>
                  <TableCell className="text-muted-foreground w-8 p-1 text-center" aria-hidden>
                    <ArrowRight className="inline h-4 w-4" />
                  </TableCell>
                  <TableCell className={`font-medium ${cellChanged(box, 'productName')}`}>
                    {getVal(box, 'productName', 'to') !== '—'
                      ? getVal(box, 'productName', 'to')
                      : (box.productName ??
                        (box.productId != null ? `Producto #${box.productId}` : '—'))}
                  </TableCell>
                  <TableCell className={cellChanged(box, 'lot')}>
                    {getVal(box, 'lot', 'to') !== '—' ? getVal(box, 'lot', 'to') : (box.lot ?? '—')}
                  </TableCell>
                  <TableCell className={cellChanged(box, 'netWeight')}>
                    {fmtWeight(getVal(box, 'netWeight', 'to'))}
                  </TableCell>
                </TableRow>
              ))}
            </TableBody>
          ...
          <TableBody>
            {boxes.map((box, i) => (
              <TableRow key={box.boxId ?? i}>
                ...
                <TableCell className="max-w-[120px] truncate font-mono text-xs" title={box.gs1128}>
                  {box.gs1128 ?? '—'}
                </TableCell>
              </TableRow>
            ))}
...
function DetailPalletUpdated({ d }) {
  ...
  if (
    d.observations != null &&
    (d.observations.from !== undefined || d.observations.to !== undefined)
  ) {
  ...
  if (Array.isArray(d.boxesUpdated) && d.boxesUpdated.length > 0) {
    sections.push(
      <DetailBoxesTable
        key="boxesUpdated"
        boxes={d.boxesUpdated}
        variant="updated"
        title="Cajas modificadas"
      />
    );
  }
  if (d.fromReception === true) {
    sections.push(
      <div key="reception" className="flex flex-wrap items-center gap-2">
        <Badge variant="secondary">Desde recepción</Badge>
        {d.receptionId != null && (
          <span className="text-muted-foreground text-sm">Recepción #{d.receptionId}</span>
        )}
      </div>
    );
  }
  ...
  const globalTotals =
    d.afterEvent && (d.afterEvent.boxesCount != null || d.afterEvent.totalNetWeight != null)
      ...
      : (() => {
          ...
          if (legacy && (legacy.newBoxesCount != null || legacy.newTotalNetWeight != null)) {
  ...
  if (sections.length === 0 && !globalTotals) {
    return (
      <Card>
        <CardContent className="text-muted-foreground pt-4 pb-4 text-sm">Sin detalle</CardContent>
      </Card>
    );
  }
    ...
    <div className="space-y-3">
      ...
      {globalTotals && (globalTotals.count != null || globalTotals.weight != null) && (
        <div className="border-border mt-2 border-t pt-2">
          <Badge variant="secondary" className="font-normal">
            {globalTotals.count ?? '—'} cajas ·{' '}
            {globalTotals.weight != null ? formatDecimalWeight(globalTotals.weight) : '—'}
          </Badge>
        </div>
      )}
    </div>
...
function DetailObservations({ d }) {
  ...
  if (fromEmpty && toEmpty) {
    ...
    content = <span className="line-clamp-2 whitespace-pre-wrap">{to}</span>;
  } else if (toEmpty) {
    content = (
      <span className="text-muted-foreground line-clamp-2 whitespace-pre-wrap">{from}</span>
    );
  } else {
    content = (
      <div className="flex items-stretch gap-3 text-sm">
        ...
        <div className="bg-muted/40 border-border/60 flex min-w-0 flex-1 flex-col rounded-md border px-2 py-1.5">
          <p className="line-clamp-6 break-words whitespace-pre-wrap">{to}</p>
        </div>
      </div>
    );
  }
...
function DetailGeneric({ d, action }) {
  ...
  if (entries.length === 0)
    return (
      <Card>
        <CardContent className="text-muted-foreground pt-4 pb-4 text-sm">{action}</CardContent>
      </Card>
    );
  return (
    <Card>
