...
const BoxesLabels = ({ pallet, setBoxPrinted }) => {
  ...
  return (
    <div className="h-[70vh] w-full pb-2">
      <Card className="flex h-full w-full flex-col bg-transparent">
        <CardContent className="flex h-full w-full flex-col gap-6 overflow-y-auto py-6">
          <ScrollArea
            orientation="horizontal"
            className="w-full overflow-x-auto py-2 whitespace-nowrap"
          >
            <div className="flex min-w-max gap-3">
              {newGroupedBoxes.length > 0 &&
                newGroupedBoxes.map((group) => (
                  <Card
                    key={group.id}
                    className="w-min-56 border-muted inline-flex h-full flex-shrink-0 flex-col rounded-2xl border shadow-sm"
                  >
                    <CardHeader className="space-y-2 pb-2">
                      <CardTitle className="text-warning-500 animate-pulse text-base font-semibold">
                        ¡Nuevas etiquetas disponibles!
                      </CardTitle>
                    </CardHeader>
                    <CardContent className="pt-0">
                      <div className="flex h-full items-center justify-center gap-4">
                        <div className="text-muted-foreground flex-1 text-sm leading-tight">
                          <span className="text-foreground font-medium">{group.product.name}</span>
                          <br />
                          <span className="font-mono">{group.lot}</span>
                          <br />
                          {group.count} cajas
                        </div>
                        <Button
