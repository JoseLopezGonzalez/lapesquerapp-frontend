...
export default function NFCPunchManager() {
    ...
    onSuccess: (result, { employeeName, eventType }) => {
      ...
      try {
        ...
        if (!Number.isNaN(d.getTime())) {
          timestampString = d.toLocaleString('es-ES', {
            hour: '2-digit',
            minute: '2-digit',
            second: '2-digit',
          });
  ...
  return (
    <div className="flex h-screen w-full flex-col">
      ...
      <Card className="m-0 flex flex-1 flex-col rounded-none border-0 shadow-none">
        <CardContent className="flex flex-1 flex-col p-6 md:p-8 lg:p-12">
          {/* Header integrado */}
          <div className="mb-6 md:mb-8">
            <h2 className="text-xl font-medium md:text-2xl">Fichaje Automático NFC</h2>
            <p className="text-muted-foreground text-sm md:text-base">
              Acerca el chip NFC al lector o introduce el código manualmente
            </p>
          </div>

          <div className="flex flex-1 flex-col items-center justify-between gap-8 md:flex-row md:items-center md:gap-12">
            ...
            {/* Contenido - Derecha */}
            <div className="flex h-full flex-1 flex-col items-center justify-center space-y-6 md:items-start md:space-y-8">
              {/* Icono NFC animado */}
              <motion.div
                className="relative"
              ...
              >
                ...
                <motion.div
                  className={cn(
                    'bg-background relative flex h-24 w-24 items-center justify-center rounded-full border-2 shadow-lg',
                    statusMessage?.type === 'error'
                      ? 'border-red-500'
                      : statusMessage?.type === 'success'
                        ? statusMessage.eventType === 'IN'
                  ...
                  animate={{
                    scale: isProcessing ? [1, 1.1, 1] : statusMessage ? [1, 1.15, 1] : 1,
                  }}
                  transition={{
              ...
              >
                ...
                <motion.p
                  className={cn(
                    'max-w-md text-sm transition-colors',
                    statusMessage?.type === 'error'
                      ? 'text-red-500'
                      : statusMessage?.type === 'success'
                  ...
                  {statusMessage?.type === 'error' ? (
                    ...
                    <>
                      ...
                      {statusMessage.timestampString && (
                        <span className="text-foreground mt-2 block text-xl font-medium">
                          {statusMessage.timestampString}
                        </span>
                      )}
                    </>
            ...
            <input
              ...
              className="pointer-events-auto absolute top-0 left-0 h-0 w-0 opacity-0"
      ...
      <Dialog
        open={!!dialogContent && !dialogContent.isError && !dialogContent.isSuccess}
        onOpenChange={() => {}}
      >
          ...
          {dialogContent && !dialogContent.isError && !dialogContent.isSuccess && (
            <>
              <DialogTitle className="sr-only">
                Registrando {dialogContent.eventTypeLabel.toLowerCase()} para{' '}
