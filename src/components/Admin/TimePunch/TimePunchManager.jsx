...
export default function TimePunchManager() {
  ...
  } = useEmployeesWithLastPunch({ perPage: 100, with_last_punch: true });
  ...
  const punchMutation = useMutation({
    mutationFn: async ({ employeeId }) => {
      ...
      const result = await createPunch({ employee_id: employeeId, device_id: DEVICE_ID }, token);
  ...
  const getLastPunchTimestamp = (lastPunchEvent) => {
    ...
    return (
      lastPunchEvent.timestamp || lastPunchEvent.created_at || lastPunchEvent.createdAt || null
    );
  };
            ...
            <div className="flex min-h-[300px] flex-col items-center justify-center py-12">
              <div className="relative">
                ...
                <div className="bg-background relative flex h-14 w-14 items-center justify-center rounded-full border shadow-xs">
                  <Users className="text-primary h-6 w-6" strokeWidth={1.5} />
                </div>
              </div>
              ...
              <p className="text-muted-foreground mt-2 max-w-[300px] text-center text-xs whitespace-normal">
                Añade empleados desde la sección de gestión de empleados para poder registrar sus
                fichajes.
              </p>
            </div>
          </CardContent>
            ...
            return (
              <Card
                ...
                className={cn(
                  ...
                  isRegistering && 'pointer-events-none opacity-75'
                )}
                  ...
                  <div className="space-y-4">
                    ...
                    {lastPunchEvent && lastPunchEventType && (
                      <div className="space-y-1 border-t pt-2">
                        ...
                        {lastPunchDate && (
                          <p className="text-muted-foreground pl-5 text-xs">{lastPunchDate}</p>
                        )}
                      </div>
                    )}
                    ...
                    >
                      ...
                      {isRegistering && <Loader2 className="text-primary h-4 w-4 animate-spin" />}
                    </div>
                  </div>
                </CardContent>
              </Card>
          ...
          <div className="space-y-3">
            <div className="flex items-start gap-3">
              ...
              <div className="space-y-2 text-sm">
                <p className="text-foreground font-medium">Cómo funciona el sistema</p>
                <ul className="text-muted-foreground list-inside list-disc space-y-1.5">
                  <li>Toca la tarjeta de un empleado para registrar su fichaje automáticamente</li>
                  <li>
                    El sistema determina si es <strong>Entrada</strong> o <strong>Salida</strong>{' '}
                    según el último evento registrado
                  </li>
                  <li>
                    Si es el primer fichaje del día o el último fue salida, se registrará una
                    entrada
                  </li>
                  <li>El fichaje se registra con la hora actual del servidor</li>
                </ul>
              </div>
            </div>
          </div>
          ...
          {dialogContent ? (
            <>
              <DialogTitle className="sr-only">
                {dialogContent.eventType === 'IN' ? 'Entrada' : 'Salida'} registrada para{' '}
              ...
              <div className="flex flex-col items-center justify-center space-y-4 p-6 text-center">
                ...
                <div className="space-y-2">
                  ...
                  <p className="text-primary text-lg font-semibold">{dialogContent.employeeName}</p>
