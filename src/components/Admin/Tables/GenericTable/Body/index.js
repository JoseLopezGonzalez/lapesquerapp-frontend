...
export const Body = ({table , data}) => {
  const { headers } = table;
    ...
    return (
        <div div className='grow overflow-y-auto overflow-x-auto w-full' >
                        ...
                        data.rows.map((row, rowIndex) => (
                            ...
                            >
                                {headers.map((header) => (
                                    <td
                                        key={header.name}
                                        className={`px-6 py-3 ${header.type === "text" ? "text-white" : ""
                                            }`}
                                    >
                                        ...
                                        {header.type === "button" && (
                                            <div className="flex gap-2">
                                                {Object.keys(row[header.name]).map((action) => (
                                                    <button
                                                        key={action}
                                                        className={`py-1 px-3 rounded-md ${action === "delete"
