...
import { Package, Clock, CheckCircle2, AlertCircle, Play, Pause } from 'lucide-react';
import { useProductionViewData } from '@/hooks/orders/useProductionViewData';
...
interface ProductionOrderItem {
  ...
  status?: 'exceeded' | 'completed' | 'pending' | string;
...
interface ProductionViewProps {
  onClickOrder?: (orderId: number | string) => void;
...
const ProductionView = ({
  ...
  const { data: rawProductionData, isLoading, error } = useProductionViewData();
  const productionData = rawProductionData as unknown as ProductGroup[] | undefined;
  ...
  const getLineStatusConfig = (orderItem: ProductionOrderItem): LineStatusConfig => {
      ...
      <div className="flex h-full w-full items-center justify-center">
        <div className="space-y-2 text-center">
          <p className="text-destructive">
            {error?.message || 'Error al obtener los datos de producción'}
          </p>
  ...
  const totalQuantity = currentProduct.orders.reduce(
  ...
  const sortedOrders = [...currentProduct.orders].sort((a, b) => {
    const dateA = a.loadDate ? new Date(a.loadDate).getTime() : 0;
    const dateB = b.loadDate ? new Date(b.loadDate).getTime() : 0;
    return dateA - dateB;
  });
