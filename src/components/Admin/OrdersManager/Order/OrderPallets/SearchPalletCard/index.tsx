...
interface SearchPalletCardProductSummary {
  product?: { name?: string } | null;
  availableNetWeight?: number | string;
  availableBoxCount?: number | string;
}

interface SearchPalletCardBox {
  lot?: string | null;
  [key: string]: unknown;
}

export interface SearchPalletCardData {
  id: number | string;
  orderId?: number | string | null;
  productsSummary?: SearchPalletCardProductSummary[];
  productsNames?: string[];
  lots?: string[];
  boxes?: SearchPalletCardBox[];
...
interface SearchPalletCardProps {
  pallet: SearchPalletCardData;
  isSelected?: boolean;
  isLinkedToOtherOrder?: boolean;
...
export default function SearchPalletCard({
  ...
  let productsSummaryArray: ProductSummary[] = [];
  if (
    ...
    productsSummaryArray = pallet.productsSummary.map((item) => ({
      ...
      netWeight: parseFloat(String(item.availableNetWeight || 0)),
      boxCount: parseInt(String(item.availableBoxCount || 0)),
    }));
  ...
  const lots =
    pallet.lots && Array.isArray(pallet.lots) && pallet.lots.length > 0
      ...
      : pallet.boxes && Array.isArray(pallet.boxes)
        ? [...new Set(pallet.boxes.map((box) => box.lot).filter(Boolean) as string[])]
        : [];
