...
const plannedProductSchema = z.object({
  ...
  boxes: z
    .union([z.number().int().min(1, 'Cajas debe ser al menos 1'), z.string()])
    .pipe(z.coerce.number().int().min(1, 'Cajas debe ser al menos 1')),
  unitPrice: z
    .union([z.number().min(0.01, 'Precio debe ser mayor que 0'), z.string()])
    .pipe(z.coerce.number().min(0.01, 'Precio debe ser mayor que 0')),
  tax: z
