...
export const AutocompleteFilter = ({ label, placeholder, endpoint, onAdd, onDelete, value }) => {
  ...
  const loadOptions = useCallback(async () => {
    ...
    try {
      ...
      const errorMessage =
        ...
        error.message ||
        `Error al cargar las opciones para ${label}. Por favor, inténtelo de nuevo.`;
      notify.error({
  ...
  const handleOnAdd = (itemValue) => {
    // Renombrado a itemValue para claridad
      ...
      <div className="col-span-2">
        <div className="flex w-full flex-col text-start">
          <label
            ...
            className="text-muted-foreground mb-2 w-full text-xs font-medium"
          >
            {label}
          </label>
            ...
            value?.map((item) => (
              <Badge key={item.id} className="flex items-center gap-1">
                ...
                <button
                  ...
                  type="button"
                  className="text-md rounded-full bg-black/20 font-bold text-black shadow-sm hover:bg-white/95"
                >
