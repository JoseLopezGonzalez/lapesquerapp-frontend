...
const PositionPopover = ({ position }) => {
  ...
  const fondoClasses = (palletId) =>
    isPalletRelevant(palletId)
    ...
    <TooltipProvider>
      <Card className="border-0 shadow-none">
        ...
        <CardContent className="py-0">
          <div
            className={`${pallets.length === 0 ? 'h-20' : pallets.length <= 3 ? 'h-auto' : 'h-[160px]'}`}
          >
              ...
              <div className="flex flex-col gap-2 py-2">
                {pallets.length === 0 ? (
                  ...
                  pallets.map((pallet) => (
                    <div
                      key={pallet.id}
                      className={`flex items-center justify-between rounded-lg border p-2 ${fondoClasses(pallet.id)}`}
                    >
