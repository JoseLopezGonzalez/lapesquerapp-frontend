...
export function AppSidebar() {
  ...
  const filteredNavigationConfig = React.useMemo(
    () => filterNavigationByFeatures(filterNavigationByRoles(navigationConfig, roles), features),
    [roles, features]
  );

  const filteredNavigationManagerConfig = React.useMemo(
    () =>
      filterNavigationByFeatures(filterNavigationByRoles(navigationManagerConfig, roles), features),
    [roles, features]
  );
  ...
  const data = React.useMemo(
    () => ({
      ...
      navigationItems: filteredNavigationConfig.map((item) =>
        isActiveRoute(item.href, currentPath) ? { ...item, current: true } : item
      ),
