...
export default function ReceptionSummaryDialog({
  ...
  return (
    <Dialog open={isOpen} onOpenChange={onClose}>
      <DialogContent size="6xl" className="flex max-h-[90vh] flex-col overflow-hidden">
        <DialogHeader>
        ...
        <Tabs defaultValue="products" className="flex flex-1 flex-col overflow-hidden">
          ...
          <TabsContent value="products" className="mt-4 flex flex-1 flex-col overflow-hidden">
            <div className="mb-4 flex items-center space-x-2">
              ...
              />
              <Label htmlFor="group-by-product" className="cursor-pointer text-sm font-normal">
                      ...
                      return (
                        <TableRow key={index}>
                          ...
                          <TableCell className="text-right align-top">
                            {formatDecimalWeight(item.totalNetWeight)}
                          </TableCell>
                          <TableCell className="text-right align-top">
                            <div className="flex justify-end">
                              {!groupByProduct ? (
                                ...
                                (() => {
                                  ...
                                  const displayPrice =
                                    priceFromMap !== undefined && priceFromMap !== null
                                      ? typeof priceFromMap === 'number'
                                        ? priceFromMap.toString()
                                        : String(priceFromMap)
                                      : '';
                  ...
                  {productLotSummary.length > 0 && (
                    <TableRow className="bg-muted/50 font-semibold">
                      ...
                      <TableCell className="text-right font-semibold">
                        {productsTotals.totalBoxes}
                      </TableCell>
                      <TableCell className="text-right font-semibold">
                        {formatDecimalWeight(productsTotals.totalWeight)}
                      </TableCell>
                      <TableCell className="text-right font-semibold">
                        {productsTotals.averagePrice > 0
                          ? `${formatDecimal(productsTotals.averagePrice)} €/kg`
                          : '-'}
                      </TableCell>
                      <TableCell className="text-right font-semibold">
                        {formatDecimal(productsTotals.totalAmount)} €
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
          ...
          {/* Pallets List Tab */}
          <TabsContent value="pallets" className="mt-4 flex flex-1 flex-col overflow-hidden">
            <div className="flex-1 overflow-auto rounded-md border">
              <Table>
                <TableHeader>
                  <TableRow>
                ...
                <TableBody>
                  {pallets.length === 0 ? (
                    ...
                    pallets.map((item, index) => {
                      ...
                      (pallet?.boxes || []).forEach((box) => {
                        if (box.product?.id) {
                          const productName =
                            box.product.name || box.product.alias || 'Producto sin nombre';
                  ...
                  {pallets.length > 0 && (
                    <TableRow className="bg-muted/50 font-semibold">
                      ...
                      <TableCell></TableCell>
                      <TableCell className="text-right font-semibold">
                        {palletsTotals.totalBoxes}
                      </TableCell>
                      <TableCell className="text-right font-semibold">
                        {formatDecimalWeight(palletsTotals.totalWeight)}
                      </TableCell>
                    </TableRow>
                  )}
                </TableBody>
