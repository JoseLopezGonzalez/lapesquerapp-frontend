    ...
    <div className="bg-muted/30 rounded-lg border px-3 py-2">
      <div className="flex items-center justify-between gap-3">
        <div className="flex min-w-0 items-center gap-2">
          <div className="bg-background shrink-0 rounded-md p-1.5">
...
function BoxDetailRow({ box, tab, showLot = true }) {
  ...
  return (
    <TableRow className="text-xs">
      <TableCell className="text-muted-foreground font-mono">{box.id}</TableCell>
...
function ProductRow({ product, boxesByProduct, costInput, onCostChange, tab }) {
  ...
  const lotEntries = useMemo(
    () =>
      Object.entries(boxesByLot).sort(([a], [b]) =>
  ...
  return (
    <>
      <TableRow className="hover:bg-muted/40 cursor-pointer" onClick={() => setOpen((o) => !o)}>
              ...
              <Table>
                ...
                <TableBody>
                  {lotEntries.map(([lot, lotBoxes]) => {
                    ...
                    const lotWeight = lotBoxes.reduce(
                      (sum, box) => sum + (Number(box.netWeightKg) || 0),
                      0
                    );
  ...
  return (
    <>
      <TableRow className="hover:bg-muted/40 cursor-pointer" onClick={() => setOpen((o) => !o)}>
        ...
        <TableCell>
          <span className="bg-muted rounded px-1.5 py-0.5 font-mono text-xs">{lot}</span>
        </TableCell>
          ...
          <div className="space-y-3 py-2">
            ...
            {result.updatedCount != null && (
              <p className="text-muted-foreground text-sm">
                {result.updatedCount}{' '}
                {result.updatedCount === 1 ? 'caja actualizada' : 'cajas actualizadas'}
              </p>
            )}
            {result.errors?.length > 0 && (
              <Alert variant="destructive">
                ...
                <AlertDescription>
                  {result.errors.length} {result.errors.length === 1 ? 'error' : 'errores'} al
                  procesar algunas cajas.
                </AlertDescription>
              </Alert>
            )}
          </div>
            ...
            {eligible.length === 0 ? (
              ...
              <>
                <div className="space-y-1.5">
                  ...
                  <div className="max-h-60 divide-y overflow-y-auto rounded-md border text-sm">
                    {byProduct
                ...
                <p className="text-muted-foreground text-sm">
                  Total: <strong>{affectedBoxes}</strong> {affectedBoxes === 1 ? 'caja' : 'cajas'}{' '}
                  en <strong>{eligible.length}</strong>{' '}
                  {byProduct
                    ? eligible.length === 1
                      ? 'producto'
                      : 'productos'
                ...
                {needsConfirmation && (
                  <Alert variant="destructive">
                    <AlertCircle className="h-4 w-4" />
                    <AlertDescription>
                      Estás a punto de modificar más de 100 cajas. Esta acción no se puede deshacer
                      fácilmente.
                    </AlertDescription>
                  </Alert>
                )}
...
function ResultsSection({
  ...
  const countLabel =
    tab === 'sales'
      ...
      : {
          icon: Warehouse,
          label: 'Palets afectados',
          value: summary?.palletsCount ?? summary?.ordersCount,
        };
  ...
  return (
    <div className="mt-3 flex min-h-0 flex-1 flex-col gap-3">
      ...
      {products.length === 0 ? (
        ...
        <Card className="flex min-h-0 flex-1 flex-col gap-2">
...
export default function CostRegularizationClient() {
  ...
  async function fetchSales() {
    ...
    try {
      setSalesData(
        await costRegularizationService.getSalesMissingCost({
          dateFrom: salesFrom,
          dateTo: salesTo,
        })
      );
  ...
  async function fetchStock() {
    ...
    try {
      setStockData(
        await costRegularizationService.getStockMissingCost({
  ...
  const filledCount =
    activeApplyMode === 'product' ? productsWithCost.length : lotProductsWithCost.length;
    ...
    <div className="flex min-h-0 flex-1 flex-col gap-4">
      ...
      <div className="shrink-0">
        ...
        <p className="text-muted-foreground mt-1 text-sm">
          Localiza cajas sin coste calculable y asigna costes manuales por producto o por producto +
          lote.
        </p>
      </div>
      ...
      <Tabs value={tab} onValueChange={setTab} className="flex min-h-0 flex-1 flex-col">
        ...
        <TabsContent value="sales" className="mt-0 flex min-h-0 flex-1 flex-col pt-4">
          <TabPanel
        ...
        <TabsContent value="stock" className="mt-0 flex min-h-0 flex-1 flex-col pt-4">
          <TabPanel
