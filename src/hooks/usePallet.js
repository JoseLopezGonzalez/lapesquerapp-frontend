...
const getInitialBoxCreationData = (preserveDiscounts = false, currentDiscounts = null) => {
  return {
    ...
    palletWeight:
      preserveDiscounts && currentDiscounts?.palletWeight !== undefined
        ? currentDiscounts.palletWeight
        : '', // peso del palet/soporte de madera para descontar en promedio - sin memoria
    showPalletWeight:
      preserveDiscounts && currentDiscounts?.showPalletWeight !== undefined
        ? currentDiscounts.showPalletWeight
        : false, // mostrar/ocultar campo de peso del palet - sin memoria
    boxTare:
      preserveDiscounts && currentDiscounts?.boxTare !== undefined ? currentDiscounts.boxTare : '', // tara de cada caja para descontar en promedio - sin memoria
    showBoxTare:
      preserveDiscounts && currentDiscounts?.showBoxTare !== undefined
...
export function usePallet({
  ...
  const temporalProductsSummary =
    temporalPallet?.boxes?.reduce((acc, box) => {
  ...
  const temporalTotalProducts =
    temporalProductsSummary && Object.keys(temporalProductsSummary).length;
  ...
  const editBox = {
    product: (boxId, product) => {
      ...
      setTemporalPallet((prev) =>
        recalculatePalletStats({
          ...prev,
          boxes: prev.boxes.map((box) => (box.id === boxId ? { ...box, product: product } : box)),
        })
      );
    },
    lot: (boxId, lot) => {
      ...
      setTemporalPallet((prev) =>
        recalculatePalletStats({
          ...
          boxes: prev.boxes.map((box) =>
            box.id === boxId
              ? { ...box, lot: lot, gs1128: getGs1128(box.product.id, lot, box.netWeight) }
              : box
          ),
        })
      );
  ...
  const bulkEditBoxes = {
    ...
    changeProduct: (boxIds, oldProductId, newProductId) => {
      ...
      const boxesToEdit = boxIds
        ...
        : temporalPallet.boxes.filter(
            (box) => box.isAvailable !== false && box.product?.id === oldProductId
          );
  ...
  const onResetBoxCreationData = () => {
    ...
    setBoxCreationData((prev) => {
      ...
      return {
        ...
        showPalletWeight:
          prev.showPalletWeight !== undefined ? prev.showPalletWeight : reset.showPalletWeight,
