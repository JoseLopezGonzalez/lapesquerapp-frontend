...
const mergeOrderDetails = (plannedProductDetails, productionProductDetails) => {
  ...
  productionProductDetails?.forEach((production) => {
    ...
    if (existing) {
      ...
      existing.status =
        existing.quantityDifference == 0
          ? 'success'
          : existing.quantityDifference <= 30 && existing.quantityDifference >= -30
            ? 'difference'
            : 'pending';
    } else {
...
const normalizePlannedProductDetail = (
  ...
  const productName =
    detail?.product?.name ?? fallbackDetail?.product?.name ?? matchedProduct?.label ?? '';

  const taxId =
    detail?.tax?.id ?? detail?.taxId ?? fallbackDetail?.tax?.id ?? fallbackDetail?.taxId ?? null;
...
export function useOrder(orderId, onChange) {
  ...
  const pallets = useMemo(() => (order?.pallets || []).map(normalizeOrderPallet), [order?.pallets]);
  ...
  useEffect(() => {
    ...
    if (
      ordersManagerOptions.productOptions?.length > 0 &&
      ordersManagerOptions.taxOptions?.length > 0
    ) {
  ...
  const onUnlinkAllPallets = async (palletIds) => {
    ...
    try {
      ...
      const errorMessage =
        ...
        error.response?.data?.userMessage ||
        error.message ||
        'Error al desvincular los palets';
      notify.error({ title: errorMessage });
      throw error;
    }
  };
