...
export function useProductionRecord(productionId, recordId = null, onRefresh = null) {
  ...
  const recordOptionsKey = productionQueryKeys.recordOptions(
    tenantId,
    productionId,
    currentRecordId
  );
  ...
  const saveMutation = useMutation({
    mutationFn: async ({ formData, isEdit }) => {
      ...
      const recordData = {
        ...
        parent_record_id:
          formData.parent_record_id && formData.parent_record_id !== 'none'
  ...
  const loading =
    productionLoading ||
    processesLoading ||
    (!!currentRecordId && recordQuery.isLoading) ||
    existingRecordsQuery.isLoading;
