...
export function useSalespeopleList(params: { filters?: CatalogListFilters; page?: number; perPage?: number; enabled?: boolean } = {}) {
  const { filters = {}, page = 1, perPage = 12, enabled = true } = params;
  const tenantId = typeof window !== 'undefined' ? getCurrentTenant() : null;
  const { data: response, isLoading, error, refetch } = useQuery({
    queryKey: ['salespeople', 'list', tenantId ?? 'unknown', filters, page, perPage],
    queryFn: () => salespersonService.list(filters, { page, perPage }),
