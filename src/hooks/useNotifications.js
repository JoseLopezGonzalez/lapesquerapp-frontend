...
export const useNotifications = () => {
  ...
  const showNotification = useCallback((message, type = NOTIFICATION_TYPES.INFO, options = {}) => {
    const { duration = 5000, onClose = null, actionLabel = null, onAction = null } = options;
  ...
  const handleApiError = useCallback(
    (error, defaultMessage = 'Ha ocurrido un error') => {
...
const NotificationToast = ({ notification, onRemove }) => {
  ...
  const getStyles = () => {
    const baseStyles =
      'p-4 rounded-lg shadow-lg border flex items-start gap-3 animate-in slide-in-from-right';
  ...
  return (
    <div className={getStyles()}>
      <span className="flex-shrink-0 text-lg font-semibold">{getIcon()}</span>
