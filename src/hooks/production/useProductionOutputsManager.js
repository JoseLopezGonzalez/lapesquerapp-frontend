...
function normalizeSourceForManageBaseline(source) {
  ...
  return {
    ...
    product_id:
      source.product_id != null && source.product_id !== '' ? String(source.product_id) : null,
    production_output_consumption_id:
      source.production_output_consumption_id != null &&
 ...
 */
export function useProductionOutputsManager({
  productionRecordId,
  initialOutputsProp = [],
  onRefresh,
}) {
          ...
          setAvailableConsumptions(
            sourcesDataResponse.parentOutputs.map((output) => ({
              ...
              productionOutputConsumptionId:
                output.productionOutputConsumptionId || output.production_output_consumption_id,
          ...
          normalizedSources = output.sources
            .map((source) => {
              ...
              return {
                ...
                product_id: source.product_id || source.productId || source.product?.id || null,
                production_output_consumption_id:
                  source.production_output_consumption_id ||
                ...
                contribution_percentage: (() => {
                  ...
                  const transformedWeight =
                    parseFloat(contributedWeight || 0) * processTransformationFactor;
                  return parseFloat(weight || 0) > 0
                    ? (transformedWeight / parseFloat(weight || 0)) * 100
                    : null;
                })(),
              };
            })
    ...
    try {
      ...
      if (allNewRows.length === 0) {
        if (useParent && useStock)
          toast.error(
            'No se encontraron productos nuevos para agregar desde las fuentes seleccionadas.'
          );
        else if (useParent)
          toast.error(
            'Todos los productos de los consumos del padre ya están añadidos como salidas.'
          );
        else if (useStock)
          toast.error(
            'No hay productos disponibles en stock o todos ya están añadidos como salidas.'
          );
        return;
      }
  ...
  const handleSaveAll = async () => {
    try {
      ...
      const allOutputs = [
        ...
        if (row.sources && Array.isArray(row.sources) && row.sources.length > 0) {
          const validSources = row.sources.filter((source) => {
            ...
            if (source.source_type === 'parent_output' && !source.production_output_consumption_id)
              return false;
            return (
              (source.contributed_weight_kg !== null &&
                source.contributed_weight_kg !== undefined &&
      ...
      const response = await syncProductionOutputs(
        productionRecordId,
        { outputs: allOutputs },
        token
      );
