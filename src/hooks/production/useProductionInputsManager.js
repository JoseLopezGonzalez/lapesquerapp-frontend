 ...
 */
export function useProductionInputsManager({
  productionRecordId,
  initialInputsProp = [],
  onRefresh,
}) {
  ...
  const inputsQuery = useQuery({
    ...
    queryFn: async () => {
      ...
      if (!token) return [];
      const response = await getProductionInputs(token, {
  ...
  const editableInputBoxIds = useMemo(
    () =>
      new Set(
        inputs.map((input) => Number(input?.box?.id)).filter((id) => Number.isFinite(id) && id > 0)
      ),
    [inputs]
  );

  const isBoxAvailable = useCallback(
    (box) => {
  ...
  const wasPreviouslyConsumed = useCallback(
    (boxId) => {
  ...
  const handleRemovePallet = useCallback(
    (palletId) => {
  ...
  const getPalletBoxes = useCallback(
    (palletId) => {
  ...
  const getSelectedBoxesForPallet = useCallback(
    (palletId) => selectedBoxes.filter((box) => box.palletId === palletId),
    [selectedBoxes]
  );
  ...
  const summaryByPallet = useMemo(() => {
    ...
    return Object.values(summaryByPallet).map((pallet) => ({
      ...
      productsBreakdown: Object.values(pallet.productsBreakdown)
        .map((product) => ({
          ...product,
      ...
      if (inputCostPerKg !== null && inputCostPerKg !== undefined) {
        ...
        if (productsMap[productName].totalWeight > 0) {
          productsMap[productName].costPerKg =
            productsMap[productName].totalCost / productsMap[productName].totalWeight;
        }
      }
  ...
  const totalSummary = useMemo(() => {
    ...
    const totalWeight = inputs.reduce(
      (sum, input) => sum + parseFloat(input.box?.netWeight || 0),
      0
    );
  ...
  const handleCalculateByWeight = useCallback(
    (palletId) => {
      const pallet = loadedPallets.find((p) => p.id === palletId);
  ...
  const calculateWeightByPallet = useCallback(
    (palletId) => {
      const allBoxes = getAllBoxes();
  ...
  const hasSelectionChanges = useMemo(() => {
    ...
    const selectedBoxIds = new Set(
      selectedBoxes.map((box) => Number(box?.boxId)).filter((id) => Number.isFinite(id) && id > 0)
    );
