...
describe('routeStops', () => {
  ...
  it('keeps stable identities for draft stops and deterministic fallback ids', () => {
    ...
    const deterministicStop = normalizeRouteStop(
      { label: 'Sin id', address: 'Puerto', position: 2 },
      1
    );
    const deterministicStopAgain = normalizeRouteStop(
      { label: 'Sin id', address: 'Puerto', position: 2 },
      1
    );
  ...
  it('changes coordinate signature only when coordinates change', () => {
    ...
    const sameCoordinates = buildStopsCoordinateSignature([
      { id: 1, label: 'B', lat: 1, lng: 2, notes: 'y', status: 'completed' },
    ]);
    const differentCoordinates = buildStopsCoordinateSignature([
      { id: 1, label: 'B', lat: 3, lng: 4 },
    ]);
