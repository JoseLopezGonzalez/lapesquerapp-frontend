...
export async function POST(req) {
    try {
        ...
        try {
            ...
            console.error('[ChatGPT Extraction] JSON parse error. Raw response (first 500 chars):', text?.substring(0, 500));
            return Response.json(
                { success: false, error: 'El modelo devolvió una respuesta no válida. Inténtelo de nuevo.' },
                { status: 422 }
            );
        }
