    ...
    return (
        <div className="h-full">    
            {loading ? (
                ...
                // 📋 Formulario
                <div className="flex flex-col w-full h-full p-4 sm:p-10 bg-neutral-800 rounded-lg shadow-lg overflow-y-auto">
                    <h1 className="text-2xl text-white p-2 font-semibold">{title}</h1>
                    <form onSubmit={handleSubmit} className="grid grid-cols-1 sm:grid-cols-6 gap-5 p-5">
                        {fields.map((field) => (
                            <div key={field.name} className="sm:col-span-3 p-2">
