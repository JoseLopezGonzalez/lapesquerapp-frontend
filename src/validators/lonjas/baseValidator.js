...
export class BaseValidator {
  ...
  requireField(value, fieldName, context = '') {
    if (value === null || value === undefined) {
      ...
      throw new ValidationError(`Campo requerido faltante: ${contextMsg}${fieldName}`, fieldName, {
        context,
      });
    }
  }
  ...
  requireArray(value, fieldName, context = '') {
    ...
    if (!Array.isArray(value)) {
      const contextMsg = context ? `${context}.` : '';
      throw new ValidationError(`Campo debe ser un array: ${contextMsg}${fieldName}`, fieldName, {
        context,
        expectedType: 'array',
  ...
  requireObject(value, fieldName, context = '') {
    ...
    if (typeof value !== 'object' || Array.isArray(value) || value === null) {
      const contextMsg = context ? `${context}.` : '';
      throw new ValidationError(`Campo debe ser un objeto: ${contextMsg}${fieldName}`, fieldName, {
        context,
        expectedType: 'object',
        actualType: typeof value,
      });
  ...
  requireNumber(value, fieldName, context = '') {
    ...
    if (typeof value !== 'number' || isNaN(value)) {
      const contextMsg = context ? `${context}.` : '';
      throw new ValidationError(`Campo debe ser un número: ${contextMsg}${fieldName}`, fieldName, {
        context,
        expectedType: 'number',
        actualType: typeof value,
      });
  ...
  requireNestedField(obj, path, context = '') {
    ...
    for (let i = 0; i < path.length; i++) {
      ...
      if (current[key] === null || current[key] === undefined) {
        const contextMsg = context ? `${context}.` : '';
        throw new ValidationError(`Campo anidado faltante: ${contextMsg}${fullPath}`, fullPath, {
          context,
          path,
        });
      }
