...
function validateDocument(document, index, validator) {
  ...
  validator.requireNonEmptyArray(
    document.tables.servicios,
    'servicios',
  ...
  document.tables.servicios.forEach((row, rowIndex) => {
    validator.requireField(
      row.Código,
      'Código',
      `document[${index}].tables.servicios[${rowIndex}]`
    );
    validator.requireField(
      row.Descripción,
      'Descripción',
  ...
  validator.requireObject(
    document.objects.subtotales_pesca,
    'subtotales_pesca',
    `document[${index}].objects`
  );
  validator.requireObject(
    document.objects.subtotales_pesca.columna,
    'columna',
    `document[${index}].objects.subtotales_pesca`
  );
  validator.requireField(
    document.objects.subtotales_pesca.columna.total_pesca,
    'total_pesca',
    `document[${index}].objects.subtotales_pesca.columna`
  );
  validator.requireField(
    document.objects.subtotales_pesca.columna.iva_pesca,
    'iva_pesca',
    `document[${index}].objects.subtotales_pesca.columna`
  );
  validator.requireField(
    document.objects.subtotales_pesca.columna.total,
    'total',
    `document[${index}].objects.subtotales_pesca.columna`
  );
  ...
  validator.requireObject(
    document.objects.subtotales_servicios,
    'subtotales_servicios',
    `document[${index}].objects`
  );
  validator.requireObject(
    document.objects.subtotales_servicios.columna,
    'columna',
    `document[${index}].objects.subtotales_servicios`
  );
  validator.requireField(
    document.objects.subtotales_servicios.columna.servicios,
    'servicios',
    `document[${index}].objects.subtotales_servicios.columna`
  );
  validator.requireField(
    document.objects.subtotales_servicios.columna.iva_servicios,
    'iva_servicios',
    `document[${index}].objects.subtotales_servicios.columna`
  );
  validator.requireField(
    document.objects.subtotales_servicios.columna.total,
    'total',
    `document[${index}].objects.subtotales_servicios.columna`
  );
  ...
  validator.requireObject(
    document.objects.subtotales_cajas,
    'subtotales_cajas',
    `document[${index}].objects`
  );
  validator.requireObject(
    document.objects.subtotales_cajas.columna,
    'columna',
    `document[${index}].objects.subtotales_cajas`
  );
  validator.requireField(
    document.objects.subtotales_cajas.columna.cajas,
    'cajas',
    `document[${index}].objects.subtotales_cajas.columna`
  );
  validator.requireField(
    document.objects.subtotales_cajas.columna.iva_cajas,
    'iva_cajas',
    `document[${index}].objects.subtotales_cajas.columna`
  );
  validator.requireField(
    document.objects.subtotales_cajas.columna.total,
    'total',
    `document[${index}].objects.subtotales_cajas.columna`
  );
}
