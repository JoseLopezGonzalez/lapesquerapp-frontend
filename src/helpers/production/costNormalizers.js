...
export const normalizeProductionCost = (cost) => {
  ...
  return {
    ...
    costCatalog: cost.cost_catalog
      ? normalizeCostCatalog(cost.cost_catalog)
      : cost.costCatalog
        ? normalizeCostCatalog(cost.costCatalog)
        : null,
    ...
    effectiveTotalCost:
      cost.effective_total_cost !== undefined
        ? cost.effective_total_cost
        : cost.effectiveTotalCost !== undefined
...
export const normalizeCostBreakdown = (breakdown) => {
  ...
  const normalizeCostTypeBreakdown = (typeBreakdown) => {
    ...
    return {
      totalCost:
        typeBreakdown.total_cost !== undefined ? typeBreakdown.total_cost : typeBreakdown.totalCost,
      costPerKg:
        typeBreakdown.cost_per_kg !== undefined
          ? typeBreakdown.cost_per_kg
          : typeBreakdown.costPerKg,
  ...
  return {
    materials: {
      totalCost:
        breakdown.materials?.total_cost !== undefined
          ? breakdown.materials.total_cost
          : breakdown.materials?.totalCost,
      costPerKg:
        breakdown.materials?.cost_per_kg !== undefined
          ? breakdown.materials.cost_per_kg
          : breakdown.materials?.costPerKg,
    ...
    processCosts: {
      production: normalizeCostTypeBreakdown(
        breakdown.process_costs?.production || breakdown.processCosts?.production
      ),
      labor: normalizeCostTypeBreakdown(
        breakdown.process_costs?.labor || breakdown.processCosts?.labor
      ),
      operational: normalizeCostTypeBreakdown(
        breakdown.process_costs?.operational || breakdown.processCosts?.operational
      ),
      packaging: normalizeCostTypeBreakdown(
        breakdown.process_costs?.packaging || breakdown.processCosts?.packaging
      ),
      total: {
        totalCost:
          breakdown.process_costs?.total?.total_cost !== undefined
            ? breakdown.process_costs.total.total_cost
            : breakdown.processCosts?.total?.totalCost,
        costPerKg:
          breakdown.process_costs?.total?.cost_per_kg !== undefined
    ...
    productionCosts: {
      production: normalizeCostTypeBreakdown(
        breakdown.production_costs?.production || breakdown.productionCosts?.production
      ),
      labor: normalizeCostTypeBreakdown(
        breakdown.production_costs?.labor || breakdown.productionCosts?.labor
      ),
      operational: normalizeCostTypeBreakdown(
        breakdown.production_costs?.operational || breakdown.productionCosts?.operational
      ),
      packaging: normalizeCostTypeBreakdown(
        breakdown.production_costs?.packaging || breakdown.productionCosts?.packaging
      ),
      total: {
        totalCost:
          breakdown.production_costs?.total?.total_cost !== undefined
            ? breakdown.production_costs.total.total_cost
            : breakdown.productionCosts?.total?.totalCost,
        costPerKg:
          breakdown.production_costs?.total?.cost_per_kg !== undefined
    ...
    total: {
      totalCost:
        breakdown.total?.total_cost !== undefined
          ? breakdown.total.total_cost
          : breakdown.total?.totalCost,
      costPerKg:
        breakdown.total?.cost_per_kg !== undefined
          ? breakdown.total.cost_per_kg
          : breakdown.total?.costPerKg,
    },
  };
};
