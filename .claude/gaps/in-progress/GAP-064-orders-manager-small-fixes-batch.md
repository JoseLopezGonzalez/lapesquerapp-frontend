...
- [x] `customerService.ts`: `getCustomerOrderHistory` y `getCustomerOrderHistoryRanges` no
      reciben `token` como parámetro — lo obtienen internamente con `getAuthToken()`
- [x] `useCustomerOrderHistoryRanges.js` no importa `useSession` ni extrae `accessToken`
- [x] `src/__tests__/hooks/useCustomerHistory.test.js` actualizado y en verde
- [x] `orderListKeys.active` existe en `src/lib/routes/queryKeys.ts`
- [x] `useOrders.js` usa `orderListKeys.active(tenantId)` — sin array literal inline
- [x] `src/app/admin/orders/[id]/page.js` no importa `Order` sin usarlo
- [x] `npm run type-check` y `npm run lint` pasan sin errores
- [x] El historial de pedidos de cliente (CRM) y el listado de pedidos activos siguen
      funcionando igual
...
- `src/services/customerService.ts` — `getAuthToken()` interno en historial/rangos
- `src/hooks/useCustomerOrderHistoryRanges.js` — eliminado `useSession`/token y deps huérfanas
- `src/__tests__/hooks/useCustomerHistory.test.js` — índice de options en mock call; mock useSession eliminado
- `src/lib/routes/queryKeys.ts` — añadido `orderListKeys.active`
- `src/hooks/useOrders.js` — usa `orderListKeys.active(tenantId)`
- `src/app/admin/orders/[id]/page.js` — import muerto de `Order` eliminado
- `src/__tests__/hooks/useOrders.test.js` — queryKey actualizado; aserción token obsoleta corregida

### Decisiones tomadas durante la implementación

- Se adoptó la factory `orderListKeys.active` con segmento `'active'` tal como define el GAP; la invalidación en `OrdersManager` usa el `queryKey` expuesto por el hook, por lo que no hay desalineación interna.
- Se corrigió en el mismo test de `useOrders` la aserción `getActiveOrders('test-token')` (obsoleta desde GAP-056) para que la suite quede en verde.
