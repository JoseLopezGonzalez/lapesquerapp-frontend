...
- [x] `OrdersManagerOptionsContext.jsx` no importa `useSession`
- [x] `OrdersManagerOptionsContext.jsx` no tiene `useState` para `productOptions` ni `taxOptions`
- [x] `OrdersManagerOptionsContext.jsx` no tiene `useEffect` para cargar opciones
- [x] Los datos de producto e impuestos se cargan con `useQuery` con `staleTime: 10 * 60 * 1000`
- [x] Los taxOptions tienen `value: string` (no `number`) en el contexto
- [x] `productOptionKeys` y `taxOptionKeys` existen en `queryKeys.ts`
- [x] `useOrderOptions.ts` sigue funcionando sin cambios (si la API del contexto no cambia)
- [x] `npm run lint` pasa sin warnings en los archivos modificados
...
- `src/context/gestor-options/OrdersManagerOptionsContext.jsx`
- `src/lib/routes/queryKeys.ts`

### Decisiones tomadas durante la implementación

- Se mantuvo el `OrdersManagerOptionsContext` y su API pública (`productOptions`, `taxOptions`,
  `productsLoading`, `taxOptionsLoading`, `loading`) para no romper consumidores existentes
  como `useOrderOptions.ts`.
- Se migró la carga de datos del provider a TanStack Query con dos `useQuery` (productos e
  impuestos), usando `enabled: !!tenantId` y `staleTime: 10 * 60 * 1000` para catálogos de
  referencia.
- Se eliminó el patrón token-as-parameter en el contexto:
  - `getProductOptions()` ya funciona con token interno.
  - Para impuestos se usa `taxService.getOptions()` (token interno via service layer).
- Se corrigió PL-018 en `taxOptions` mapeando `value` como `String(tax.value)`.
- En `queryKeys.ts` se añadió `taxOptionKeys.options(...)` y se normalizó
  `productOptionKeys` con `options(...)`, manteniendo `list(...)` como alias por
  compatibilidad.
...
- `npx eslint src/context/gestor-options/OrdersManagerOptionsContext.jsx src/lib/routes/queryKeys.ts` ✅
...
- La API del contexto se mantiene intacta (`productOptions`, `taxOptions`, flags de loading,
  `loading` agregado). `useOrderOptions.ts` no requirió cambios.
- Impuestos ahora usan `taxService.getOptions()` (endpoint `/taxes/options`) en lugar del
  legacy `getTaxOptions(token)` con formato `{ id, rate }`. El label pasa a ser el del
  endpoint de opciones (`tax.label`) en vez de `` `${rate} %` `` — comportamiento más
  alineado con el resto de selects del proyecto.
- `productOptionKeys.list()` sigue existiendo como alias de `options()` — sin ruptura en
  `useProductOptions.js`.
- Fuera de scope (no bloquea cierre): `useTaxOptions.js` mantiene fallback con token-as-parameter
  cuando no hay contexto del gestor — candidato para un GAP futuro.

### Estado final de la implementación

GAP cerrado. Refactor completo del provider a TanStack Query, token-as-parameter eliminado
del contexto, PL-018 corregido en `taxOptions.value`.
