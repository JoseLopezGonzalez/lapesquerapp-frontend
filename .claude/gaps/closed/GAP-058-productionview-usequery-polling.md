...
- [x] `src/hooks/orders/useProductionViewData.ts` existe con `useQuery` y `refetchInterval: 3 * 60 * 1000`
- [x] `productionViewKeys` factory existe en `src/lib/routes/queryKeys.ts`
- [x] `ProductionView/index.js` no importa `useSession`
- [x] `ProductionView/index.js` no tiene `useState` para datos del servidor
- [x] `ProductionView/index.js` no tiene `useEffect` / `setInterval` para polling
- [x] El `console.log` de la línea ~53 eliminado
- [x] El polling de 3 minutos sigue funcionando (via `refetchInterval`)
- [x] Estados de loading y error se muestran correctamente en la UI
...
- `src/hooks/orders/useProductionViewData.ts` — hook con `useQuery`, `refetchInterval: 3min`, `staleTime: 1min`

### Archivos modificados

- `src/lib/routes/queryKeys.ts` — añadida factory `productionViewKeys.data(tenantId)`
- `src/components/Admin/OrdersManager/ProductionView/index.js` — eliminados token, useState/useEffect de fetch y polling manual; usa `useProductionViewData()`

### Decisiones tomadas durante la implementación

- Se eliminó también `notify.error` del callback manual de fetch: el error se muestra en la UI vía `error?.message` del hook, coherente con el patrón de `useCustomersList`.
- El polling silencioso del intervalo manual queda cubierto por `refetchInterval` de TanStack Query (no activa `isLoading` en refetches).
...
- El `setInterval` que queda en `ProductionView` (línea 66) es solo para auto-play de navegación entre productos, no para polling de datos — correcto y fuera del scope del GAP.
- Warning ESLint preexistente en `ProductionView` (`setCurrentIndex` en `useEffect`, regla `react-hooks/set-state-in-effect`) — no introducido por este GAP; candidato a limpieza futura.
- Se eliminó `notify.error` en errores de fetch (documentado en implementación): el error solo se muestra en pantalla. Comportamiento ligeramente distinto al anterior pero coherente con hooks de listado del proyecto.

### Estado final de la implementación

GAP cerrado. Refactor completo: token eliminado del componente, estado del servidor migrado a `useProductionViewData` con `refetchInterval` de 3 minutos y factory `productionViewKeys` registrada.
