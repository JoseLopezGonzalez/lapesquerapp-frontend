...
- `src/components/LandingPage/HowItWorks.tsx` — reescrito: quitados los iconos Lucide
  por paso, sustituidos por `AssetPlaceholder` tipo 1 con los `label` exactos del GAP;
  añadida línea conectora (horizontal en desktop vía `border-t` + `inset-x-[16.67%]`,
  vertical en mobile vía `border-l` centrado).
- `src/components/LandingPage/TrustBadge.tsx` — reescrito: de 1 icono+título+descripción
  a 3 claims (icono + título + descripción cada uno) en grid `sm:grid-cols-3`, sección
  envuelta en `bg-muted/30`; comentario `// TODO` documentando los 2 claims bloqueados
  (GS1-128, RGPD) y sus prerrequisitos. No se renombró el archivo (`TrustBadge.tsx` se
  mantiene, opción marcada como no bloqueante en el GAP) para minimizar el diff.
- `src/messages/es/landing.json`, `pt/landing.json`, `en/landing.json` — reescritos
  `howItWorks.step1/step2/step3` (título+descripción) y `trustBadge` completo (de
  `description` única a `title` + `claims.labeling/traceability/faoCatalog`).

### Decisiones tomadas durante la implementación

- **Bug real encontrado y corregido durante la verificación visual (no estaba en el
  plan del GAP):** la línea conectora de `HowItWorks` no era visible en la primera
  implementación. Causa raíz: el contenedor `relative` que envuelve la línea (`-z-10`)
  y el grid de pasos no establecía su propio _stacking context_ (un `position:relative`
  sin `z-index` explícito no lo hace), así que el hijo con `-z-10` escapaba hacia arriba
  en el árbol de stacking contexts y terminaba pintándose detrás del fondo de la
  `<section>`, invisible. Corregido añadiendo `isolate` al contenedor (fuerza un
  stacking context local sin necesidad de `z-index` numérico), verificado visualmente
  con captura real tras el fix: la línea aparece correctamente detrás de los 3 círculos
  numerados, en desktop y mobile.
- Los claims 4 (GS1-128) y 5 (RGPD) se dejaron exclusivamente como comentario de código
  en `TrustBadge.tsx` — no se creó ninguna clave de traducción para ellos (ni siquiera
  sin usar), para que sea imposible activarlos por error.

### Desviaciones del plan (si las hay)

Ninguna respecto al contenido/claims acordados en `landing-proposal.md` §12.4/§12.5. El
fix del stacking context es una corrección de bug encontrado durante la propia
implementación, no una desviación del plan.
...
- [x] `HowItWorks.tsx` sin `Fish`/`Boxes`/`ShoppingCart` — usa `AssetPlaceholder` tipo 1
      con los 3 `label` especificados.
- [x] Línea conectora visible entre los 3 pasos en desktop y variante vertical en mobile
      — verificado visualmente tras corregir el bug de stacking context (ver arriba).
- [x] Copy de los 3 pasos (ES) centrado en el pedido — "El pedido, el centro de todo" /
      "Producción y stock reales" / "Documentación, envío y rentabilidad".
- [x] `TrustBadge.tsx` muestra 3 items (icono + título + descripción).
- [x] Los 3 claims activos mencionan Reglamento UE 1379/2013, trazabilidad por
      palet/caja/lote, catálogo FAO/ASFIS (+13.700 especies).
- [x] `grep -rn "GS1\|RGPD\|GDPR"` sobre el componente y sus claves de `landing.json` →
      0 apariciones en contenido renderizado (solo en comentario `// TODO`).
- [x] `TrustBadge` tiene fondo `bg-muted/30`.
- [x] `grep` de cifras/certificaciones no verificadas → 0.
- [x] Paridad de claves verificada entre los 3 `landing.json`.
- [x] `GET /es` devuelve 200 con ambas secciones renderizadas correctamente — verificado
      con Playwright.
- [x] Cero regresión sobre el resto de la home.
...
Verificado con Playwright real, desktop + mobile: línea conectora visible en ambos
breakpoints (tras el fix de `isolate`), 3 claims de `TrustBadge` bien alineados con
icono/título/descripción, sin colores hardcodeados, sin `sky-*`.

### Observaciones para Jose

**⚠️ Hallazgo importante, fuera del alcance de este GAP pero descubierto durante su
verificación:** al probar `prefers-reduced-motion: reduce` con un navegador real
(Playwright, no solo lectura de código), se detectó que **`ScrollReveal.tsx` no respeta
esa preferencia en la práctica** — `useReducedMotion()` de `framer-motion` sí detecta
correctamente la preferencia (`window.matchMedia` confirma `true`), pero el componente
no llega a renderizar la rama `<div>` sin animación: el elemento se queda con
`opacity:0` permanentemente (nunca se revela) porque la condición no vuelve a
evaluarse/aplicarse a tiempo tras el primer render. **Esto afecta a todas las secciones
de la landing que usan `ScrollReveal`** (todas, desde B2), no solo a las tocadas en esta
ronda — es un bug pre-existente, no introducido por este GAP ni por GAP-132/134.
`ScrollReveal.tsx` no está en la lista de archivos de ningún GAP de esta ronda, así que
no se ha tocado — señalado aquí y en `landing-proposal.md` para que se abra un GAP corto
específico. Es el motivo de la puntuación 9/10 en vez de 10/10: el hallazgo es real y
relevante pero está fuera del alcance acordado para este GAP.

### Estado final de la implementación

Completo y funcionando. `HowItWorks` con capturas tipo 1 + línea conectora real,
`TrustBadge` con 3 claims verificados y 2 bloqueados correctamente documentados como
deuda técnica sin exponerse en ningún sitio.
